...
$(document).ready(function(){
    ...
    $(document).on("click",".order_product_edit",function(){
        ...
        var old_cnt = $tr.find("td.order_count").text();

        $tr.find("td.order_count").html("<input class='form-data' name='cnt'  value='"+old_cnt+"'>");
    ...
    $(document).on("click",".btn_save_out_order_complete",function(){
        ...
        // ajax 처리 부분 * 
        var url = "./contents/fl/out_order/api/completeOutOrderStatus.php";
    ...
    $(document).on("click",".btn_order_product_edit_cancel",function(){
        ...
        var $tr = $(this).closest("tr");
        var old_cnt = $tr.find("td.order_count").attr("old_value");

        $tr.find("td.order_count").html(old_cnt);
    ...
    $(document).on("click",".btn_order_product_edit_save",function(){
        ...
        modifyOrderProduct(out_order_part,oocp_idx,cnt,"edit");
    ...
    $(document).on("click","#datatable-main tr td:not(.out_order_status)",function(){
        ...
        var url = "./contents/fl/out_order/api/getOrderInfo.php";
        var out_order_idx = $(this).closest("tr").attr("out_order_idx");
                ...
                success: function(result){
                    ...
                    if(result_status == 1)
                    {
                        ...
                        for(var i=0;i<order_product_list.length;i++){
                            ...
                            $tr_sample.find(".product_name").html(order_product_list[i].product_name);
                            ...
                            $tr_sample.find(".total_client_price_sum").html(number_format(order_product_list[i].total_client_price_sum)+" 원");
...
function saveOocpStatus(mode,out_order_idx){
    ...
    if(mode == "배송요청"){
        ...
        var str = "mode="+mode+"&out_order_idx="+out_order_idx+"&storage_idx="+storage_idx; //
    ...
    var url = "./contents/fl/out_order/api/save_product_output.php";
...
function modifyOrderProduct(out_order_part,oocp_idx,cnt,mode){
    ...
    if(mode == "del"){
        ...
        var str = "mode="+mode+"&oocp_idx="+oocp_idx+"&cnt="+cnt+"&out_order_part="+out_order_part; //

    }

        
    var url = "./contents/fl/out_order/api/modifyOrderProduct.php";
...
function goNextOutOrderStatus(out_order_idx,out_order_status){
    ...
    var url = "./contents/fl/out_order/api/updateCell.php";
