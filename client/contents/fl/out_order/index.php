...
$main_filter_query = " consulting_idx='".$manager_info['consulting_idx']."' and out_order_status != '주문취소' ";
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["발주담당자","manager_name","like",["consulting.manager"],["manager_idx","manager_idx"]]);   
  ...
  $keyword_column_array_selected = "r_name";      //기본 선택옵션 value
...
array_push($th_info,["manager_name","발주요청인"]);
...
$other_table_column = "a:date_format(regist_datetime,'%Y-%m-%d %H;%i') as reg_datetime+date_format(r_date,'%m-%d') as r_md+t_company_name as company_name/a:manager_idx:consulting.manager:manager_name";
  ...
  switch (column) {
    ...
    case "company_name":
        ...
        if(full.out_order_status != "접수대기" && full.out_order_status != "주문취소"){
          rValue += "<br><span class='blue'>"+full.t_storage_name.substr(0,8)+"</span>";

        }else if(full.out_order_status == "접수대기" && full.return_datetime != ""){
          rValue += "<br><span class='red'>반려:"+full.return_storage_name.substr(0,8)+"</span>";
    ...
    case "out_order_status":
        if(data == "접수대기"){
          rValue = '<button type="button" class="btn btn-info">접수대기</button>';
        }else if(data == "배송요청" || data == "본부접수" || data == "주문접수"){
          rValue = '<button type="button" class="btn btn-warning">주문확인</button>';
        }else if(data == "배송중"){
          rValue = '<button type="button" class="btn btn-primary">배송중</button>';
...
<script src="./contents/board/js/lightbox.js"></script>
<script>
    lightbox.option({
      'resizeDuration': 10,
      'wrapAround': true,
