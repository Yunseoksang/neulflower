...
$(document).ready(function(){
    ...
    $(document).on("click",".btn_add_client_order",function(){
       window.location.href='dashboard_sffm.php?page=fullfillment/client_input/order';
    });
    ...
    $(document).on("click","#datatable-main tr td",function(){
        ...
        var url = "./contents/fu/out_order/api/getOrderInfo.php";
        ...
        $.ajax( { 
                ...
                success: function(result){
                    ...
                    if(result_status == 1)
                    {
                        ...
                        $("#detail_section").find(".total_client_price").html(number_format(order_info.total_client_price)+" 원");
                        $("#detail_section").find(".total_client_price_tax").html(number_format(order_info.total_client_price_tax)+" 원");
                        $("#detail_section").find(".total_client_price_sum").html(number_format(order_info.total_client_price_sum)+" 원");
                        ...
                        if(order_info.io_status == "미출고" || order_info.io_status == undefined){
                            $("#detail_section").find(".btn_cancel_out_order").show();

                        }else{
                            $("#detail_section").find(".btn_cancel_out_order").hide();
    ...
    $(document).on("click", ".btn_io_status",function() {
        ...
        var url = "./contents/fu/storage_out/api/updateCell.php";
...
function saveOocpStatus(mode,oocp_idx,storage_idx,admin_memo){
    ...
    var url = "./contents/fu/out_order/api/save_product_output.php";

    //var str = "mode="+mode+"&oocp_idx="+oocp_idx+"&storage_idx="+storage_idx+"&admin_memo="+admin_memo; //
    var str = "mode="+mode+"&oocp_idx="+oocp_idx+"&storage_idx="+storage_idx; //
