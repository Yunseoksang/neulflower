...
$folder_name = "fu/out_order";  //폴더명
$title ="물품 발주내역";

$table_name ="fullfillment.out_order_client_product";
...
$main_filter_query = " consulting_idx='".$manager_info['consulting_idx']."' and oocp_status != '주문취소' ";
...
array_push($th_info,["order_count","수량"]);
array_push($th_info,["io_status","출고상태"]);
array_push($th_info,["manager_name","발주자"]);
...
$other_table_column = "a:consulting_idx:consulting.consulting:company_name/a:out_order_idx:fullfillment.in_out:io_status/a:out_order_idx:fullfillment.out_order:manager_idx/fullfillment.out_order:manager_idx:consulting.manager:manager_name";  //a:out_order_idx:out_order:to_place_name+to_address+to_name+to_hp+memo/
...
array_push($css_array,["product_name",["min-width","150px"]]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    case "io_status":
      ...
      if(data == ""){
        rValue = '<button type="button" class="btn btn-info "  >주문접수</button>';

      }else if(data == "미출고"){
        rValue = '<button type="button" class="btn btn-warning "  >발송대기</button>';

      }else if(data == "출고완료"){
        rValue = '<button type="button" class="btn btn-success " >출고완료</button>';
