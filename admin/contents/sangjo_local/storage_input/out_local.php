        ...
        <div class="right_set" mode='minus'>
          <div class="page-title">
            <div class="title_left">
              <h3>
                    출고서 작성  : <?=$admin_info['t_storage_fullfillment']?>
                    <small>
...
left join (
    ...
    select * from in_out where io_idx in (select max(io_idx) as max_io_idx from in_out where storage_idx='".$admin_info['storage_fullfillment']."' group by product_idx)
) b
on a.product_idx=b.product_idx 

left join (
    select *,sum(out_count) as sum_out_count from in_out where io_status='이동출고완료' and to_storage_idx='".$admin_info['storage_fullfillment']."' group by product_idx
) c
