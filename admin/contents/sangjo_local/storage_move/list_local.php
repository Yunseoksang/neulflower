...
$folder_name = "sfullfillment_local/storage_move";  //폴더명
$title ="[".$admin_info['t_storage_fullfillment']."] 이동내역";
...
$main_filter_query = " (storage_idx='".$admin_info['storage_fullfillment']."' and  ((part='이동출고' and io_status='이동출고완료')  or (part='이동입고' and io_status='이동입고완료')) )";
...
if($this_agent == "pc"){
    ...
    array_push($th_info,["t_storage_name","출발지창고"]);
    array_push($th_info,["t_to_storage_name","도착지창고"]);


    array_push($th_info,["t_product_name","품목"]);
    array_push($th_info,["out_count","이동수량"]);
...
$opt = ":--처리상태--/이동출고완료/이동입고완료";  //"0:노출중단/1:노출"
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    case "exec_detail":
        ...
        rValue += "<div class='detail_div hide'>"+full.io_idx+" | " +full.t_from_storage_name+" → "+full.t_storage_name+" | 요청일:" +full.regist_datetime.split(" ")[0]+" | 수령일:" +full.receive_date+" | "+full.memo;
        rValue += "</div>";
        break;
    case "io_idx":
          rValue = data;
    ...
    case "io_status":
          if(data == "이동출고완료"){
            rValue = '<button type="button" class="btn btn-success">'+data+'</button>';

          }else if(data == "이동입고완료"){
            rValue = '<button type="button" class="btn btn-primary">'+data+'</button>';
          }
    ...
    case "t_storage_name": //출발지창고
        if(full.part == "이동출고"){
          rValue = data;
        }else{
          rValue = full.t_from_storage_name;
        }

        break;
    case "t_to_storage_name": //도착지창고
        if(full.part == "이동출고"){
          rValue = data;
        }else{
