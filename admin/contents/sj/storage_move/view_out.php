...
$main_filter_query = " ( part='이동출고' and io_status = '미출고'  )";  //and DATEDIFF(now(),receive_date) < 1



$function_add_custom = ["on","btn_download_xls_moveout","엑셀다운로드"];    // 커스텀 추가하기 버튼 on ,off
$function_add_html = "<input type='text' class='form-control' id='barcode_input_all' placeholder='QR이동출고' style='width:150px; float:right;'>";
...
array_push($th_info,["manager","도착지관리자"]);
array_push($th_info,["hp","전화번호"]);
...
//array_push($th_info,["exec_td","실행","exec2"]);
...
array_push($css_array,["regist_datetime",["max-width","100px"]]);
array_push($css_array,["update_datetime",["max-width","100px"]]);
array_push($css_array,["exec",["min-width","100px"]]);

array_push($css_array,["address",["min-width","150px"]]);
...
$opt = ":--처리상태--/미출고/이동출고완료";  //"0:노출중단/1:노출"
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "regist_datetime":
        ...
        break;
    case "io_status":
        rValue = '<button type="button" class="btn btn-danger btn_io_status" next_io_status="이동출고완료">출고하기</button>';
        rValue += '<br><button type="button" class="btn btn-info btn_qr_moveout "  next_io_status="이동출고완료">QR이동출고</button>';
      break;

    case "exec_td":
        rValue = '<button type="button" class="btn btn-dark btn_modify"  >수정</button>';
        rValue += '<button type="button" class="btn btn-primary btn_io_status_cancel" next_io_status="이동출고취소">취소하기</button>';
