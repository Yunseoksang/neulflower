...
$folder_name = "sj/storage_move";  //폴더명
$title ="이동입고현황";
...
$main_filter_query = " ( part='이동입고' and io_status = '미입고'  )";  //and DATEDIFF(now(),receive_date) < 1


$function_add_custom = ["on","btn_download_xls_movein","엑셀다운로드"];    // 커스텀 추가하기 버튼 on ,off
$function_add_html = "<input type='text' class='form-control' id='barcode_input_all' placeholder='QR이동입고' style='width:150px; float:right;'>";
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["출발지창고명","t_storage_name","like"]);   //로딩시 첫 옵션 기본선택됨. dataTables.ajax.button.js 에서 keyword_query = setKeywordQuery(0); 에 의해 첫번째 옵션이 기본옵션으로 선택됨.
  array_push($keyword_column_array,["도착지창고명","t_to_storage_name","like"]);
...
if($this_agent == "pc"){
  ...
  array_push($th_info,["regist_datetime","요청일자"]);


  array_push($th_info,["t_from_storage_name","출발지창고"]);
  array_push($th_info,["t_storage_name","도착지창고"]);


  array_push($th_info,["t_product_name","품목"]);
  array_push($th_info,["in_count","이동수량"]);




  array_push($th_info,["out_date","발송일자"]);
  array_push($th_info,["receive_date","수령일자"]);


  array_push($th_info,["admin_memo","메모"]);

  //array_push($th_info,["t_write_admin_name","등록관리자"]);
  //array_push($th_info,["t_out_manager_name","발송관리자"]);
  //array_push($th_info,["t_in_manager_name","수령관리자"]);
  ...
  array_push($th_info,["io_status","처리"]);
  array_push($th_info,["exec","실행","exec2"]);
  ...
  /*th 헤더 정보 입력 */
  $th_info = [];
  array_push($th_info,["exec_detail","상세"]);
  ...
  $unsortable_columns = ["exec_detail","out_date","t_product_name","in_count","io_status"];  //soring 기능 불가칼럼 지정.
  $order_by_column = "io_idx";
...
//array_push($css_array,["exec",["min-width","100px"]]);
...
array_push($edit_array,["receive_date","date"]);
array_push($edit_array,["admin_memo","textarea"]);


// //selectDropDown
$io_status_array = make_options_array("-처리상태/미출고/이동출고완료/이동입고완료");
array_push($edit_array,["io_status","select",["options",$io_status_array],["direct_save","1"],["same","Y"]]);

// //selectDropDown
// $delivery_type_array = make_options_array("-배송방법-/택배/직배송");
// array_push($edit_array,["delivery_type","select",["options",$delivery_type_array],["direct_save","1"],["same","Y"]]);
...
$opt = ":--처리상태--/미입고/이동입고완료";  //"0:노출중단/1:노출"
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "exec_detail":
        rValue = "<i class='fa fa-plus-circle btn_show_detail'></i><i class='fa fa-minus-circle btn_hide_detail hide'></i>";
        rValue += "<div class='detail_div hide'>"+full.io_idx+" | from:" +full.t_from_storage_name+" | to:"+full.t_storage_name+" | "+full.memo;
        rValue += "</div>";
        break;
    case "io_idx":
          rValue = data;
    ...
    case "regist_datetime":
        ...
        var fullDate = new Date(Date.parse(data));
        var mm = ("0" + (fullDate.getMonth() + 1)).slice(-2);
        var dd = ("0" + fullDate.getDate()).slice(-2);

        rValue = fullDate.getFullYear()+"-"+mm+"-"+dd;
        break;

    case "io_status":
        rValue = '<button type="button" class="btn btn-warning btn_io_status" next_io_status="이동입고완료">입고하기</button>';
        rValue += '<br><button type="button" class="btn btn-info btn_qr_movein "  next_io_status="이동입고완료">QR이동입고</button>';
        break;

    case "exec_td":
        rValue = '<button type="button" class="btn btn-dark btn_modify"  >수정</button>';
        // rValue += '<button type="button" class="btn btn-primary btn_io_status_cancel" next_io_status="이동입고취소">취소하기</button>';
