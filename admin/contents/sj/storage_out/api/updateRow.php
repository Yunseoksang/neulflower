...
if($jsonData->mode == "update"){
   ...
   if($set_sql2 != ""){
      $sel = mysqli_query($dbcon, "select * from in_out where ".$key_name."='".$key_value."' ") or die(mysqli_error($dbcon));
      $sel_num = mysqli_num_rows($sel);
      
      if ($sel_num > 0) {
         $data = mysqli_fetch_assoc($sel);

         $sql_2 = "update out_order set ".$set_sql2." where out_order_idx=".$data['out_order_idx'];

         $up = mysqli_query($dbcon, $sql_2) or die(mysqli_error($dbcon));
      }else{
