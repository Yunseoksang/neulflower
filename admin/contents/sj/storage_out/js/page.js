...
$(document).ready(function(){
    ...
    $(document).on("click",".btn_download_xls_list",function(){
        ...
        //toast("준비중입니다");
        window.location.href="./contents/sj/storage_out/api/download_xls.php?mode=list";

    });


    $(document).on("click",".btn_download_xls_view",function(){

        //toast("준비중입니다");
        window.location.href="./contents/sj/storage_out/api/download_xls.php?mode=view";
    ...
    $(document).on("click", ".btn_io_status",function() {
        ...
        var url = "./contents/sj/storage_out/api/updateCell.php";
        var io_idx = $(this).closest("tr").attr("io_idx");
                ...
                success: function(result){
                    ...
                    if(result_status == 1)
                    {
                        ...
                        if(io_status == "출고완료"){
                            ...
                            $(this).closest("td").find(".btn-primary").remove();
                            $(this).closest("tr").find(".btn_qr_out").remove();


                        }else if(io_status == "출고취소"){
                            $(this).removeClass("btn-primary").addClass("btn-dark").text("취소완료");
                            $(this).closest("td").find(".btn-danger").remove();
                            $(this).closest("tr").find(".btn_qr_out").remove();
...
function go_next_status(qr_code){
    ...
    var str = "mode=io_status&qr_mode=qr&qr_code="+qr_code+"&io_status="+next_io_status+"&io_idx="+io_idx; //
    console.log(str);
