...
$product_arr = array();
$sel = mysqli_query($dbcon, "select product_idx,product_name,display_order,memo,delivery_unit from product where display='on' order by display_order, product_name ") or die(mysqli_error($dbcon));
...
select sum(a.current_count) as sum_current_count,a.storage_idx,a.t_storage_name from 

(
select p.* from storage_safe p
left join product q 
on p.product_idx=q.product_idx
where q.product_idx is not null and q.display='on'
) a 

left join storage b 
on a.storage_idx=b.storage_idx

where b.storage_idx is not null

group by a.storage_idx order by a.storage_idx
    "
;
...
//재품별 재고 합계
$product_sum_sql = "
select sum(a.current_count) as sum_current_count,a.product_idx from 
...
group by a.product_idx order by a.product_idx
"
;
...
//창고별+상품별 미입고내역 합계
$storage_in_wait_sql = "
select sum(a.in_count) as sum_in,a.storage_idx,a.product_idx
...
on p.product_idx=q.product_idx
where q.product_idx is not null and q.display='on'
and p.part='이동입고' and p.io_status='미입고'
...
group by a.storage_idx,a.product_idx
...
//상품별 미입고내역 합계
$pr_in_wait_sql = "
select sum(a.in_count) as sum_in,a.product_idx from 
...
select sum(a.in_count) as sum_in,a.storage_idx from
        ...
        <div class="right_set" mode='plus'>
          <div class="page-title">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12 form-group pull-right" style="text-align:right;">
                      <a href='contents/sj/storage/api/getSpreadSheet.php'><button type="button" class="btn btn-primary btn_excel_download">엑셀 다운로드</button></a>
            </div>
          </div>
                  ...
                  <table id="datatable_static" class="table table-striped responsive-utilities jambo_table bulk_action" style="100%">
                    <thead>
                      <tr class="headings">
                        ...
                        <th class="column-title th_delivery_unit delivery_unit">배송<br>단위</th>
                    ...
                    for ($i=0;$i<count($product_arr);$i++ )
                    {
                      ...
                      ?>
                        <tr class="<?=$line_class?> pointer">
                            ...
                            <td class=" "><a href="?page=sj/storage_adjust/history&product_idx=<?=$product_arr[$i]['product_idx']?>" target="history"><?=$product_arr[$i]['product_name']?></a></td>
...
?>
                            <td class="delivery_unit"><a href="?page=sj/storage_product/list" target="product"><?=$product_arr[$i]['delivery_unit']?></td>
                            <td class="td_sum"><?=$this_pr_sum?></td>
...
<?
                            ...
                            for ($k=0;$k<count($storage_arr);$k++)
                            {
                              ...
                              ?>
                                    <td class=" <?=$safe_alert?>" title="<?=$title_alt?>"><a href="dashboard_sj.php?page=sj/storage_input/move&storage_idx=<?=$storage_arr[$k]['storage_idx']?>" target="move"><?=$storage_safe_arr[$storage_column][$product_column][1]?><?=$safe_num_w?><?=$in_sum?></a></td>
                            <?}
                            ?>
