<?
$folder_name = "framework";  //폴더명
$title ="관리자페이지 구조설정";

$table_name ="framework.page_config";
$key_column_name ="page_config_idx";
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["상품명","product_name","like"]);   //로딩시 첫 옵션 기본선택됨. dataTables.ajax.button.js 에서 keyword_query = setKeywordQuery(0); 에 의해 첫번째 옵션이 기본옵션으로 선택됨.
...
array_push($th_info,["page_config_idx","INDEX"]);
array_push($th_info,["main_db","메인DB"]);
array_push($th_info,["main_table","메인TABLE"]);
array_push($th_info,["title","페이지 타이틀"]);
array_push($th_info,["folder_name","폴더"]);
array_push($th_info,["update_key_column","업데이트키"]);
array_push($th_info,["sorting_key_column","기본정렬키"]);
array_push($th_info,["search_date","날짜검색"]);
array_push($th_info,["search_keyword","키워드검색"]);
array_push($th_info,["add_btn_new","신규추가버튼"]);
array_push($th_info,["add_btn_custom","커스텀버튼"]);
array_push($th_info,["add_custom_html","커스텀html"]);
// array_push($th_info,["regist_datetime","등록시간"]);
// array_push($th_info,["update_datetime","수정시간"]);

// $exec_colorbox_edit = "<button class='btn btn-primary btn-xs btn_colorbox_edit'  href='#inline_content'>수정</button>";
// array_push($th_info,["exec","실행","custom_exec",$exec_colorbox_edit]); //수정 + 소속/권한
...
array_push($th_info,["exec","실행","colorbox_edit"]);
...
/******************수정모드 설정************************************************************************************************************* */
//근무지는 DB에서 가져오기
$sel0 = mysqli_query($dbcon, "SHOW DATABASES") or die(mysqli_error($dbcon));
...
if ($sel_num0 > 0) {
  while($data0 = mysqli_fetch_assoc($sel0)) {
      $this_arr = array();
      array_push($this_arr,$data0['Database']);
      array_push($this_arr,$data0['Database']);
      
      //if($data0['selected'] == 1){array_push($this_arr,"selected");}

      array_push($database_array,$this_arr);      
  }   
}
...
array_push($edit_array,["main_db","select",["options",$database_array],["direct_save","1"],["same","N"]]);

array_push($edit_array,["main_table","메인TABLE","필수","text"]);
array_push($edit_array,["title","타이틀","필수","text"]);
array_push($edit_array,["folder_name","폴더","","text"]);
...
//array_push($add_array,["main_db","메인DB","필수","select",["options",$database_array],["direct_save","1"],["same","N"]]);




//array_push($add_array,["category_idx","카테고리","필수","select",[["1","상조용품","SELECTED"],["2","꽃배달"]]]); //DB column 기준이라서 brand_name 이 아니고,brand_idx로,



array_push($add_array,["main_db","메인DB","필수","select",$database_array]); //DB column 기준이라서 brand_name 이 아니고,brand_idx로,
array_push($add_array,["main_table","메인TABLE","필수","text"]);
array_push($add_array,["title","타이틀","필수","text"]);
array_push($add_array,["folder_name","폴더","","text"]);


//array_push($add_array,["product_price","가격","필수","text"]);
...
//array_push($add_array,["product_img_url","상품이미지","","image","product"]); //네번째는 저장할 디렉토리 지정가능
...
require('./contents/common/datatable_html_colorbox.php');
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "exec_colorbox_edit":
          ...
          //rValue = data;
          rValue = "<button class='btn btn-primary btn-xs btn_colorbox_edit'  href='#inline_content'>수정</button>";
          break;
