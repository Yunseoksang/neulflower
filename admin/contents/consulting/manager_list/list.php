...
$folder_name = "consulting/manager_list";  //폴더명
$title ="거래처 담당자 목록";

$table_name ="consulting.manager";
...
//$main_filter_query = " t_consulting_status='계약완료' ";


if($_REQUEST['consulting_idx'] != ""){
  $main_filter_query = "consulting.manager.consulting_idx='".$_REQUEST['consulting_idx'] ."' ";
}
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["담당자명","manager_name","like"]);   //로딩시 첫 옵션 기본선택됨. dataTables.ajax.button.js 에서 keyword_query = setKeywordQuery(0); 에 의해 첫번째 옵션이 기본옵션으로 선택됨.
  array_push($keyword_column_array,["아이디","manager_id","like"]);
  //array_push($keyword_column_array,["거래처","company_name","like"]);
  array_push($keyword_column_array,["거래처","company_name","like","join",["consulting"],["consulting_idx","consulting_idx"]]);
  array_push($keyword_column_array,["휴대폰","manager_hp","like"]);
  array_push($keyword_column_array,["회사전화","tel","like","join",["consulting"],["consulting_idx","consulting_idx"]]);
...
array_push($th_info,["manager_idx","고유번호"]);
//array_push($th_info,["consulting_idx","회사고유번호"]);
...
array_push($th_info,["company_name","거래처"]); 
array_push($th_info,["consulting_status","계약단계"]); 
...
array_push($th_info,["manager_position","직급"]);
array_push($th_info,["manager_department","부서"]);
// array_push($th_info,["manager_tel","전화"]);
// array_push($th_info,["manager_email","이메일"]);
// array_push($th_info,["manager_settlement_date","정산마감일"]);
...
$other_table_column = "a:consulting_idx:consulting.consulting:company_name+consulting_status+consulting_idx"; //메인테이블이 아닌 다른 테이블에서 join을 통해 가져와야 하는 칼럼 => sorting 할때 뒤쪽에 붙여야 함.
...
//수정모드 설정
$edit_array = [];
array_push($edit_array,["manager_id","text",["direct_save","1"]]);
array_push($edit_array,["item_in_charge","text",["direct_save","1"]]);
array_push($edit_array,["manager_name","text",["direct_save","1"]]);
...
// array_push($edit_array,["manager_settlement_date","text",["direct_save","1"]]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    //   rValue = "<a href='?page=consulting/detail&consulting_idx='"+full.consulting_idx+" target='_blank'>"+data+"</a>";
