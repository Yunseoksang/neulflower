...
$function_add_html = '<a href="?page=consulting/input&mode=sp"><button class="btn btn-primary btn_add_custom btn_add_supplier">매입처 추가</button></a>';


if($_REQUEST['part'] == "sp"){

  $main_filter_query = " part='sp'  ";


}else if($_REQUEST['status'] != ""){

  if($_REQUEST['status'] == "0"){
    $main_filter_query = "part='consulting' and (consulting_status='계약전' or consulting_status='계약전(상담)' or consulting_status='계약전(견적)' or consulting_status='계약전(계약서)') ";
    
  }else if($_REQUEST['status'] == "1"){
    $main_filter_query = "part='consulting' and  consulting_status='계약전(상담)' ";

  }else if($_REQUEST['status'] == "2"){
    $main_filter_query = "part='consulting' and  consulting_status='계약전(견적)' ";

  }else if($_REQUEST['status'] == "3"){
    $main_filter_query = "part='consulting' and  consulting_status='계약전(계약서)' ";

  }else if($_REQUEST['status'] == "4"){
    $main_filter_query = "part='consulting' and  consulting_status='계약보류' ";

  }else if($_REQUEST['status'] == "5"){
    $main_filter_query = "part='consulting' and  consulting_status='계약완료' ";

  }else if($_REQUEST['status'] == "6"){
    $main_filter_query = "part='consulting' and  consulting_status='계약종료' ";

  }else{
    $main_filter_query = "part='consulting'  ";

  }



  //$main_filter_query = " consulting_status='".$_REQUEST['status']."' ";
...
if($function_keyword_search == "on"){
  ...
  //array_push($keyword_column_array,["미팅담당자","meeting_person","like"]); 

  array_push($keyword_column_array,["본사명","head_office","like"]);
  array_push($keyword_column_array,["주소","address","like"]); 
  array_push($keyword_column_array,["전화번호","tel","like"]); 
...
$order_by_column = "update_datetime";
$order_by_sort   = "desc";
...
array_push($th_info,["company_name","업체명"]);

if($_REQUEST['part'] != "sp"){

  array_push($th_info,["office_type","본사/계열사"]);

  array_push($th_info,["employees","상시근로자수"]);
  array_push($th_info,["employment_fee","고용부담금"]);
  
}


array_push($th_info,["contract_date","계약일"]);
array_push($th_info,["trading_items","거래품목"]);


//array_push($th_info,["meeting","미팅여부"]);
...
//array_push($th_info,["consulting_status","상담단계"]);
...
//array_push($th_info,["meeting_person","미팅담당자"]);

//array_push($th_info,["regist_datetime","등록일"]);
array_push($th_info,["update_datetime","업데이트"]);

array_push($th_info,["consulting_status","진행단계","selectDropDown",[  ["계약전(상담)","계약전(상담)","btn-info"],["계약전(견적)","계약전(견적)","btn-info"],["계약전(계약서)","계약전(계약서)","btn-info"],["계약보류","계약보류","btn-warning"],["계약완료","계약완료","btn-success"],["계약종료","계약종료","btn-dark"] ] ]);
...
array_push($css_array,["company_name",["min-width","200px"]]);


if($_REQUEST['part'] == "sp"){
  array_push($css_array,["trading_items",["width","400px"]]);

}
array_push($css_array,["office_type",["width","80px"]]);

array_push($css_array,["update_datetime",["max-width","100px"]]);
array_push($css_array,["consulting_status",["min-width","140px"],["width","140px"]]);
array_push($css_array,["contract_date",["min-width","100px"]]);
...
//require('./contents/common/datatable_html_hidden_right.php');
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "company_name":
          if(full.office_type == "계열사"){
            rValue = data + "<br><span class='scom'>-"+full.head_office+"-</span>";
          }else{
            rValue = data;
          }
    ...
    case "office_type":
          ...
          if(data == "본사"){
            rValue = "<span class='sky'>"+data+"</span>";
          }else{
            rValue = data;
          }

          if(full.jisa_count > 0){
            rValue = rValue+"<span class='red'>("+full.jisa_count+")</span>";
          }
