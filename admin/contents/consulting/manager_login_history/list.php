...
$folder_name = "consulting/manager_login_history";  //폴더명
$title ="담당자 로그인 이력";

$table_name = "consulting.manager_login_history";
$key_column_name ="manager_idx";
...
if($function_date_search == "on"){
  //날짜검색 옵션 넣을 경우
  $function_date_column['regist_datetime'] = "로그인날짜";      //날짜 검색 옵션칼럼 + 보여지는문구
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["담당자명","manager_name","like","join",["consulting.manager"],["manager_idx"]]);
  array_push($keyword_column_array,["담당자아이디","manager_id","like","join",["consulting.manager"],["manager_idx"]]);
  array_push($keyword_column_array,["소속","t_storage_name","like","join",["consulting.manager"],["manager_idx"]]);
  ...
  // array_push($keyword_column_array,["등록담당자명","cf_manager_name","like"]);
  ...
  $keyword_column_array_selected = "manager_name";      //기본 선택옵션 value
...
array_push($th_info,["manager_login_history_idx","고유번호"]);



// //array_push($th_info,["storage_idx","소속"]);
// //array_push($th_info,["manager_level","권한"]);
...
// array_push($th_info,["start_page","시작페이지","convert",[    ["종합물류관리","종합물류관리"],["화훼관리","화훼관리"],["상조물류관리","상조물류관리"],["계약업체관리","계약업체관리"],["통계관리","통계관리"]    ] ]);
...
// array_push($th_info,["manager_status","승인단계","selectDropDown",[  ["승인대기","승인대기","btn-info"],["승인","승인","btn-success"],["승인거절","승인거절","btn-dark","divider"] ,["탈퇴","탈퇴","btn-dark"]  ] ]);

// array_push($th_info,["manager_login_count","로그인횟수"]);

array_push($th_info,["regist_datetime","로그인시각"]);


// $manager_exec = "<button type='button' class='btn btn-dark btn-xs btn_modify'>수정</button>
...
// array_push($th_info,["exec","실행","custom_exec",$manager_exec]); //수정 + 소속/권한



$other_table_column = "a:manager_idx:consulting.manager:manager_id+manager_name+t_storage_name"; //메인테이블이 아닌 다른 테이블에서 join을 통해 가져와야 하는 칼럼 => sorting 할때 뒤쪽에 붙여야 함.
...
//$other_table_column = "a:manager_idx:manager.manager_permission:"; //메인테이블이 아닌 다른 테이블에서 join을 통해 가져와야 하는 칼럼 => sorting 할때 뒤쪽에 붙여야 함.
...
// array_push($edit_array,["manager_name","이름","필수","text"]);
// array_push($edit_array,["manager_hp","휴대폰","필수","text"]);
// //array_push($edit_array,["manager_id","아이디","필수","text"]);

// $start_page_array = make_options_array("종합물류관리:종합물류관리/화훼관리:화훼관리/상조물류관리:상조물류관리/계약업체관리:계약업체관리/통계관리:통계관리");


// $manager_level_array = make_options_array("종합관리자:종합관리자:selected/물류관리자:물류관리자/물류창고관리자:물류창고관리자/상조관리자:상조관리자/상조창고관리자:상조창고관리자/계약업체관리자:계약업체관리자");
// $manager_status_array = make_options_array("승인:승인/승인대기:승인대기/승인거절:승인거절/탈퇴:탈퇴");
...
// array_push($edit_array,["manager_level","select",["options",$manager_level_array],["direct_save","1"],["same","Y"]]);
// array_push($edit_array,["storage_idx","select",["options",$storage_idx_array],["direct_save","1"],["same","N"]]);
// array_push($edit_array,["manager_status","select",["options",$manager_status_array],["direct_save","1"],["same","Y"]]);
...
// array_push($add_array,["manager_level","권한","","select",$manager_level_array]); //DB column 기준이라서 brand_name 이 아니고,brand_idx로,
...
// array_push($add_array,["manager_name","이름","필수","text"]);
// array_push($add_array,["manager_hp","휴대폰","","text"]);
// array_push($add_array,["manager_id","아이디","필수","text"]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    // case "manager_idx":
    //       rValue = "<span class='manager_idx'><a href='?page=manager/list&sval="+full.manager_idx+"' target='category'>"+full.manager_name+"</a></span><br><span class='etc category_name'>("+data+")</span>";
    //       break;
