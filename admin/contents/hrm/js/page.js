...
$(document).ready(function(){
    ...
    $(document).on("click",".hrm_edit",function(){

        var hrm_idx = $("table.table_hrm_key_info .hrm_name").attr("hrm_idx");


        window.location.href='dashboard_hrm.php?page=hrm/input&hrm_idx='+hrm_idx;
    ...
    $(document).on("click",".hrm_del",function(){
    ...
    $(document).on("click",".btn_show_hrm_info",function(){

        var this_status = $(this).attr('status');
        if(this_status == "show"){
            $(this).attr("status","hide");
            $("table.table_hrm_info").hide();
            $("table.table_manager_info").hide();
        }else{
            $(this).attr("status","show");

            $("table.table_hrm_info").removeClass("hide");
            $("table.table_manager_info").removeClass("hide");
    
            $("table.table_hrm_info").show();
    ...
    $(document).on("click","li.memo_li span.memo_status",function(){
        ...
        var url = "./contents/hrm/api/updateCell.php";
    ...
    $(document).on("click",".btn_save_history,.btn_save_product_history",function(){
        ...
        var url = "./contents/hrm/api/insertMemo.php";
        ...
        var hrm_idx = $("#detail_section").find(".hrm_name").attr("hrm_idx");
        ...
        if(memo_idx > 0){ //수정
            var str = "mode=edit&part="+part+"&mdate="+mdate+"&memo_idx="+memo_idx+"&hrm_idx="+hrm_idx+"&memo_status="+memo_status+"&memo_title="+title+"&memo_text="+memo; //

        }else{ //추가
            var str = "mode=add&part="+part+"&mdate="+mdate+"&hrm_idx="+hrm_idx+"&memo_status="+memo_status+"&memo_title="+title+"&memo_text="+memo; //
    ...
    $(document).on("click","div.btn-group[col_name='hrm_status'] ul.dropdown-menu li a",function(e){

        var url = "./contents/hrm/api/updateCell.php";
        var hrm_status = $(this).text();
        var hrm_idx = $(this).closest("tr").attr("hrm_idx");

        var str = "mode=hrm_status&hrm_status="+hrm_status+"&hrm_idx="+hrm_idx; //
        ...
        $.ajax( { 
                ...
                success: function(result){
                    ...
                    {
                        toast(result.msg);
                        $(this).closest("td").attr("hrm_status",hrm_status);
                    }else{
                        var msg = result.msg;
                        window.alert(msg);
                    }
...
//회사 정보 삭제
function deleteCompany(hrm_idx){

    var url = "./contents/hrm/api/updateCell.php";

    var str = "mode=del&hrm_idx="+hrm_idx; //
...
function ajaxFileUpload() {
    ...
    var hrm_idx = $("table.table_hrm_key_info").find("th.hrm_name").attr("hrm_idx");
    if(hrm_idx == undefined || hrm_idx == ""){
    ...
    var url = "./contents/hrm/api/uploadFile.php";


    var form = jQuery("ajaxFrom")[0];
    var formData = new FormData(form);
    formData.append("hrm_idx", hrm_idx);
...
function delete_attach($obj){
    ...
    var url = "./contents/hrm/api/uploadFile_del.php";
