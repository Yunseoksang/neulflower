...
//     $location = $_SERVER["DOCUMENT_ROOT"].'/upload/hrm/'.$name;
...
//$path = $_SERVER["DOCUMENT_ROOT"]."/upload/hrm/";
...
if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    {       
        ...
        $folder_idx = "/upload/hrm/".$_POST['hrm_idx'];
		...
		if(file_exists($absolute_path)){ 
            ...
            $local_name = "/upload/hrm/".$_POST['hrm_idx']."/".$new_name;
            $absolute_path = $_SERVER["DOCUMENT_ROOT"].$local_name;
        }
        ...
        if(move_uploaded_file($tmp, $absolute_path))
        {
            ...
            if($in_id){//쿼리성공
                ...
                if ($sel_num > 0) {
                    ...
                    $up = mysqli_query($dbcon, "update hrm.hrm set update_datetime=now() where hrm_idx='".$_POST['hrm_idx']."' ") or die(mysqli_error($dbcon));
