...
if($admin_info['pm_super'] == "종합관리자"){
    //
}else if($admin_info['pm_hrm'] == "인사관리자" || $admin_info['pm_hrm'] == "인사총무관리자" ){
    // 
}else{
...
$function_add_html = '<a href="?page=hrm/input"><button class="btn btn-primary btn_add_custom btn_add_supplier">직원 추가</button></a>';


if($_REQUEST['status'] == "1"){
  $main_filter_query = "hrm_status='계약중' ";
}else if($_REQUEST['status'] == "2"){
  $main_filter_query = "hrm_status='계약종료' ";
}else{
  $main_filter_query = "hrm_status='계약중'  ";
}

if($admin_info['storage_hrm'] != null && $admin_info['storage_hrm'] != "1"){ //본사가 아니면 => 지사 선택
  $main_filter_query .= " and jisa_idx='".$admin_info['storage_hrm']."'  ";
}
...
if($function_date_search == "on"){
  //날짜검색 옵션 넣을 경우
  $function_date_column['date_of_employment'] = "입사일";      //날짜 검색 옵션칼럼 + 보여지는문구
  $function_date_column['date_of_resignation'] = "퇴사일";  //날짜 검색 옵션칼럼 + 보여지는문구
  //-------------------------------------------------------------------------
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["이름","name","like"]);   //로딩시 첫 옵션 기본선택됨. dataTables.ajax.button.js 에서 keyword_query = setKeywordQuery(0); 에 의해 첫번째 옵션이 기본옵션으로 선택됨.
  //array_push($keyword_column_array,["미팅담당자","meeting_person","like"]); 
  array_push($keyword_column_array,["소속지사","t_jisa","like"]); 
  array_push($keyword_column_array,["소속기관","t_organization","like"]); 
  array_push($keyword_column_array,["직책","job_position","like"]); 
  array_push($keyword_column_array,["근무지","t_office","like"]); 
  ...
  $keyword_column_array_selected = "name";      //기본 선택옵션 value
...
array_push($th_info,["hrm_idx","고유번호"]);
array_push($th_info,["name","이름"]);
array_push($th_info,["t_jisa","소속지사"]);
array_push($th_info,["t_organization","소속기관"]);
array_push($th_info,["work_type","근로유형"]);
array_push($th_info,["t_office","근무지"]);
array_push($th_info,["date_of_employment","입사일"]);
array_push($th_info,["date_of_resignation","퇴사일"]);


array_push($th_info,["hrm_status","계약상태","selectDropDown",[ ["계약중","계약중","btn-success"],["계약종료","계약종료","btn-dark"] ] ]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "name":
          ...
          rValue = "<a href='?page=hrm/detail&hrm_idx="+full.hrm_idx+"'>"+full.name+"</a>";
