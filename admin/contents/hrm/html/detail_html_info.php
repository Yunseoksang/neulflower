...
$sel = mysqli_query($dbcon, "select * from hrm where hrm_idx='".$_REQUEST['hrm_idx']."' ") or die(mysqli_error($dbcon));
...
$sel1 = mysqli_query($dbcon, "select *,IFNULL(date_format(mdate,'%Y%m%d'),'') as md,date_format(update_datetime,'%Y-%m-%d %H:%i') as utime from memo where hrm_idx='".$_POST['hrm_idx']."' order by mdate desc, memo_idx desc") or die(mysqli_error($dbcon));
...
$sel1 = mysqli_query($dbcon, "select *,IFNULL(date_format(mdate,'%Y%m%d'),'') as md,date_format(update_datetime,'%Y-%m-%d %H:%i') as utime from memo_product where hrm_idx='".$_POST['hrm_idx']."' order by mdate desc,memo_idx desc") or die(mysqli_error($dbcon));
$sel1_num = mysqli_num_rows($sel1);
...
$sel2 = mysqli_query($dbcon, "select * from attachment where hrm_idx='".$_REQUEST['hrm_idx']."' order by attachment_idx") or die(mysqli_error($dbcon));
...
$sel3 = mysqli_query($dbcon, "select * from manager where hrm_idx='".$_REQUEST['hrm_idx']."' order by manager_idx") or die(mysqli_error($dbcon));
...
<table class="table table-bordered table_hrm_key_info" hrm_idx="<?=$_REQUEST['hrm_idx']?>">
    ...
    <tbody>
        ...
        <tr class="tr_value">
            <td class="hrm_status"><?=$company_info['hrm_status']?></td>
