...
from
(select * from sangjo.in_out where  part='출고' and io_status='출고완료' and date_format(output_datetime,'%Y-%m')='".$YM."' ) a
left join sangjo.out_order_client_product b 
on a.out_order_idx=b.sangjo_out_order_idx 
...
if ($sel_num > 0) {
  while($data = mysqli_fetch_assoc($sel)){
    ...
    $sel_y = mysqli_query($dbcon, "select * from ".$this_table_name." where consulting_idx='".$data['consulting_idx']."' and part='상조'  and yyyy='".$data['yyyy']."' ") or die(mysqli_error($dbcon));
...
$folder_name = "statistics/sj_cnst_monthly";  //폴더명
$title ="거래처별 매출현황";
...
require('./contents/statistics/sj_cnst_monthly/html/datatable_html.php');
...
function datatableCreatedCell(column,$td,rowData){
  switch (column) {
    ...
    //   }else if(rowData.out_order_part == "상조"){
    //     $td.closest("tr").attr("out_order_part","상조").addClass("tr_flower");
    //   }
