...
$sql0 = "
select date_format(a.output_datetime,'%Y%m') as yyyymm, a.consulting_idx,c.company_name,count(*) as cnt,
...
from
(select * from sangjo.in_out where  part='출고' and io_status='출고완료' and  date_format(output_datetime,'%Y%m')='".$yyyymm."'  ) a
left join sangjo.out_order_client_product b 
on a.out_order_idx=b.flower_out_order_idx 

left join consulting.consulting c 
on a.consulting_idx=c.consulting_idx

group by a.consulting_idx,yyyymm order by company_name";
...
if ($sel_num > 0) {
  while($data = mysqli_fetch_assoc($sel)){
    ...
    $sel_y = mysqli_query($dbcon, "select * from statistics.consulting_m where consulting_idx='".$data['consulting_idx']."' and part='상조' and yyyymm='".$data['yyyymm']."' ") or die(mysqli_error($dbcon));
...
$folder_name = "statistics/fu_cnst_m";  //폴더명
$title ="거래처별 월별 매출현황";
...
require('./contents/statistics/fu_cnst_m/html/datatable_html_split.php');
