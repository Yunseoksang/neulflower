...
from
(select * from fullfillment.in_out where  part='출고' and io_status='출고완료' and  date_format(output_datetime,'%Y%m')='".$yyyymm."'  ) a
...
if ($sel_num > 0) {
  while($data = mysqli_fetch_assoc($sel)){
    ...
    $sel_y = mysqli_query($dbcon, "select * from statistics.consulting_m where consulting_idx='".$data['consulting_idx']."' and part='종합물류' and yyyymm='".$data['yyyymm']."' ") or die(mysqli_error($dbcon));
