...
select a.storage_idx,a.t_storage_name,count(*) as cnt,sum(price_calcu) as sum_price from (
select out_order_idx,storage_idx,t_storage_name from flower.out_order where out_order_part='화훼' and out_order_status !='주문취소' and date_format(r_date,'%Y-%m')='".$YM."' and storage_idx is not null 
) a 
...
$sql0 = "
select a.consulting_idx,a.t_company_name,count(*) as cnt,
sum(b.total_client_price) as total_price ,
sum(b.total_client_price_tax) as total_price_tax ,
sum(b.total_client_price_sum) as total_price_sum 


from (
select out_order_idx,consulting_idx,t_company_name from flower.out_order where out_order_part='화훼' and out_order_status ='배송완료' and date_format(r_date,'%Y-%m')='".$YM."' and consulting_idx is not null ) a
left join flower.out_order_client_product b 
on a.out_order_idx=b.out_order_idx

group by a.consulting_idx order by t_company_name";
...
if ($sel_num > 0) {
  while($data = mysqli_fetch_assoc($sel)){
    ...
    $sel_y = mysqli_query($dbcon, "select * from statistics.consulting_m where consulting_idx='".$data['consulting_idx']."' and part='화훼' and yyyymm='".$yyyymm."' ") or die(mysqli_error($dbcon));
    $sel_y_num = mysqli_num_rows($sel_y);
    
    if ($sel_y_num == 0) {

      $sql1 = "insert into  statistics.consulting_m
      
      set
      consulting_idx='".$data['consulting_idx']."',
      t_company_name='".$data['t_company_name']."',
      part='화훼',
      yyyymm=".$yyyymm.",
      cnt=".$data['cnt'].",
      ...
      $sql2 = "update statistics.consulting_m
...
$folder_name = "statistics/flower_consulting_m";  //폴더명
$title ="거래처별 월별 매출현황";

$table_name ="statistics.consulting_m";
...
$main_filter_query = " part='화훼'  "; //and yyyymm='".$yyyymm."'
...
array_push($th_info,["consulting_idx","거래처명"]);
...
array_push($css_array,["price_sum",["text-align","right"]]);
...
require('./contents/statistics/flower_consulting_m/html/datatable_html_split.php');
