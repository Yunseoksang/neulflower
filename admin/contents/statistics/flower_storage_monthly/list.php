...
$this_table_name = "statistics.flower_storage_monthly";

$sel = mysqli_query($dbcon, "select storage_idx,t_company_name,count(*) as cnt,sum(total_client_price) as sum_price from flower.out_order where out_order_part='화훼' and out_order_status ='배송완료' and date_format(regist_datetime,'%Y-%m')='".$YM."' and storage_idx is not null group by storage_idx order by t_company_name") or die(mysqli_error($dbcon));
$sel_num = mysqli_num_rows($sel);

if ($sel_num > 0) {
  while($data = mysqli_fetch_assoc($sel)){
  
    $sel_y = mysqli_query($dbcon, "select * from ".$this_table_name." where yyyy='".$this_yyyy."' and flower_storage_idx='".$data['storage_idx']."' ") or die(mysqli_error($dbcon));
    ...
    if ($sel_y_num == 0) {
      ...
      yyyy='".$this_yyyy."',
      ".$colnum_name_cnt."='".$data['cnt']."',
      ".$colnum_name_price."='".$data['sum_price']."'
...
$folder_name = "statistics/flower_storage_monthly";  //폴더명
$title ="지점/협력사별 매출현황";

$table_name ="statistics.flower_storage_monthly";
...
//$other_table_column = "a:storage_idx:".$db_consulting.".consulting:company_name";
...
array_push($css_array,["storage_idx",["width","120px"]]);
...
require('./contents/statistics/flower_storage_monthly/html/datatable_html.php');
