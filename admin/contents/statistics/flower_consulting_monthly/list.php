...
$sel = mysqli_query($dbcon, "select consulting_idx,t_company_name,count(*) as cnt,sum(total_client_price) as sum_price from flower.out_order where out_order_part='화훼' and out_order_status ='배송완료' and date_format(regist_datetime,'%Y-%m')='".$YM."' and consulting_idx is not null group by consulting_idx order by t_company_name") or die(mysqli_error($dbcon));
$sel_num = mysqli_num_rows($sel);

if ($sel_num > 0) {
  while($data = mysqli_fetch_assoc($sel)){
  
    $sel_y = mysqli_query($dbcon, "select * from ".$this_table_name." where consulting_idx='".$data['consulting_idx']."' and part='화훼'  and yyyy='".$this_yyyy."' ") or die(mysqli_error($dbcon));
...
$folder_name = "statistics/flower_consulting_monthly";  //폴더명
$title ="거래처별 매출현황";
...
require('./contents/statistics/flower_consulting_monthly/html/datatable_html.php');
