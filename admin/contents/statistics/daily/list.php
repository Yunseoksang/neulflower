...
$sel_recent = mysqli_query($dbcon, "select ymd from ".$db_statistics.".sj_daily order by ymd desc limit 2,1 ") or die(mysqli_error($dbcon));
$sel_recent_num = mysqli_num_rows($sel_recent);

if ($sel_recent_num > 0) {
    $data_recent = mysqli_fetch_assoc($sel_recent);
    $s_ymd = $data_recent['ymd'];



    $in = mysqli_query($dbcon, "insert ignore into ".$db_statistics.".sj_daily (ymd,ym,product_idx,product_name,input_amount)
    ...
    t_product_name,
    sum(in_count) 
    from ".$db_fullfillment.".in_out 
    where io_status='입고완료' and input_datetime >= '".$s_ymd." 00:00:00'
...
$folder_name = "statistics/daily";  //폴더명
$title = "일별 생산량";
...
$footer_sum_display = "on";


$date_search_display = "hide";//수정없음
if($function_date_search == "on"){
  //날짜검색 옵션 넣을 경우
  $function_date_column['ymd'] = "검색날짜";      //날짜 검색 옵션칼럼 + 보여지는문구
  //$function_date_column['update_datetime'] = "내용변경일";  //날짜 검색 옵션칼럼 + 보여지는문구
  //-------------------------------------------------------------------------
  $function_date_column_selected = "ymd";      //기본 선택옵션 value
...
//   array_push($keyword_column_array,["관리자명","manager","like"]);
   array_push($keyword_column_array,["품목(일치)","product_name","="]);
   array_push($keyword_column_array,["품목(포함)","product_name","like"]);
...
array_push($th_info,["ymd","날짜"]);
array_push($th_info,["product_name","품목"]);

array_push($th_info,["input_amount","생산량"]);
array_push($th_info,["error_amount","불량"]);
array_push($th_info,["exec_error_rate","불량율"]);

// array_push($th_info,["exec_old_input_amount","기존일생산량"]);
// array_push($th_info,["exec_old_defective_amount","기존일불량"]);
// array_push($th_info,["exec_old_defective_rate","기존불량율"]);
...
array_push($edit_array,["input_amount","text",["direct_save","1"]]);

array_push($edit_array,["error_amount","text",["direct_save","1"]]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "exec_error_rate":

          if(full.error_amount != undefined){
            rValue = ((full.error_amount/full.input_amount)*100).toFixed(2) +"%";
