  ...
  <div class="">
    ...
    <div class="page-title">
      <div class="title_left">
        <h3>
              <a href="<?=$_SERVER['PHP_SELF']."?page=".$_GET['page']?>"><?=$title?></a>
              <small>
                  <!--- 칼럼 다중 보기 옵션 -->
                  <?
                  if(count($view_group_array) > 0){?>
                    <span>보기옵션:</span> 
                    <?
        ...
        <div class="x_panel">
          ...
          <div class="x_title">
            ...
            <div class="col-md-<?=isset($title_bar_ratio[1])?$title_bar_ratio[1]:''?> filter_box <?=$process_search_display?>"  style="text-align:center;">
              ...
              <?}else{?>
                  <div class="btn-group main_process_menu  <?=$process_search_display?>" data-toggle="buttons">
            ...
            <div class="col-md-<?=isset($title_bar_ratio[2])?$title_bar_ratio[2]:''?> title_right_col" style="text-align:right; padding-right:0px;">
                ...
                <button class="btn btn-primary btn_order_save">주문접수</button>
      ...
      if(is_iterable($edit_array)){
        for ($i=0;$i<count($edit_array);$i++ )
        {
            ...
            switch ($mm)
            {
                      ...
                      <select name="<?=$this_colname?>" <? if($same != ""){?>same="<?=$same?>"<?}?> class="<?=$this_colname?> form-control <?=$direct_save?>">
                          <?
                            for ($j=0;$j<count($options);$j++ )
                            {
                              //print_r($options[$j][0]); 
                              ?>
                                <option value="<?=$options[$j][0][0]?>"  <? if($options[$j][0][2] == "selected"){?>selected<?}?>  <? if($options[$j][1] != ""){?><?=$options[$j][1]?><?}?> ><?=$options[$j][0][1]?></option>
                            <?}
                          ?>
                      </select>
                  <?break;
                ...
                case "select2":?>
                    <div class="<?=$this_colname?>">
                      <div class="col-md-12 col-sm-12 col-xs-12 form-group has-feedback">
                        <select class="select2_single form-control"   <? if($same != ""){?>same="<?=$same?>"<?}?>  tabindex="-1" name="<?=$this_colname?>" style="display: none;">
                            <?
                            for ($j=0;$j<count($options);$j++ )
                            {
                ...
                case "editable":
                case "text":?>
                    <input name="<?=$edit_array[$i][0]?>" <? if($format != ""){?>format="<?=$format?>"<?}?> class="<?=$edit_array[$i][0]?> form-control <?=$direct_save?>">
