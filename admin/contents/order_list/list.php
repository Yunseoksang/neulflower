<?
$folder_name = "order_list";  //폴더명
$title ="주문목록";

$table_name ="order_list";
$key_column_name ="order_list_idx";
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["전달장소","r_place_name","like"]);   //로딩시 첫 옵션 기본선택됨. dataTables.ajax.button.js 에서 keyword_query = setKeywordQuery(0); 에 의해 첫번째 옵션이 기본옵션으로 선택됨.
  array_push($keyword_column_array,["받는분","r_name","like"]);
  array_push($keyword_column_array,["수신자","r_name","like"]);
...
array_push($th_info,["order_list_idx","주문번호"]);
//array_push($th_info,["category_idx","카테고리" ]);
array_push($th_info,["t_company_name","주문자"]);
array_push($th_info,["order_list_status","주문상태","convert",[    ["0","미확인"],["1","주문접수"],["2","발주완료"], ["3","배송완료"]    ] ]);

array_push($th_info,["r_datetime","배송요구일"]);


array_push($th_info,["order_title","주문내역"]);

array_push($th_info,["price_sum","주문금액"]);

array_push($th_info,["r_store","수주화원"]);

array_push($th_info,["r_name","받는분"]);
array_push($th_info,["r_address1","배송지"]);
array_push($th_info,["r_phone","전화번호"]);


array_push($th_info,["regist_datetime","주문접수일<br>수정일","datetime2line"]);
...
array_push($add_array,["r_name","받는분","필수","text"]);
