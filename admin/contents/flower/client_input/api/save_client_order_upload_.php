...
set_exception_handler(function($e) {
    debug_to_file("전역 예외: " . $e->getMessage() . "\n" . $e->getTraceAsString(), 'GLOBAL_EXCEPTION');
    
    $result = array();
    $result['status'] = 0;
    $result['msg'] = "처리 중 예외가 발생했습니다: " . $e->getMessage();
...
set_error_handler(function($errno, $errstr, $errfile, $errline) {
    debug_to_file("오류 발생: [$errno] $errstr - $errfile:$errline", 'GLOBAL_ERROR');
    
    if ($errno == E_ERROR || $errno == E_PARSE || $errno == E_CORE_ERROR || 
        $errno == E_COMPILE_ERROR || $errno == E_USER_ERROR) {
        $result = array();
        $result['status'] = 0;
        $result['msg'] = "처리 중 오류가 발생했습니다: $errstr";
        echo json_encode($result);
        exit;
    }
    
    return false; // 다른 오류 처리기로 전달
});

// 디버깅 시작
debug_to_file('스크립트 시작', 'START');
...
try {
    ...
    // 디버깅을 위한 로그 기록
    debug_to_file($_POST['data'], 'POST_DATA');

    // JSON 데이터 디코딩 시 오류 처리 추가
    $arr = json_decode($_POST['data'], true);
    if (json_last_error() !== JSON_ERROR_NONE) {
        debug_to_file(json_last_error_msg(), 'JSON_ERROR');
        $result = array();
        $result['status'] = 0;
        $result['msg'] = "데이터 형식 오류: " . json_last_error_msg();
        echo json_encode($result);
        exit;
    }

    debug_to_file($arr, 'DECODED_DATA');

    // 필수 데이터 확인
    if (!isset($arr['flower_product_list']) || !isset($arr['sangjo_product_list'])) {
        debug_to_file('필수 데이터 누락', 'MISSING_DATA');
        $result = array();
        $result['status'] = 0;
        $result['msg'] = "필수 데이터가 누락되었습니다.";
        echo json_encode($result);
        exit;
    }

    // 변수 초기화
    $flower_out_order_idx = isset($arr['flower_out_order_idx']) ? $arr['flower_out_order_idx'] : 0;
    $sangjo_out_order_idx = isset($arr['sangjo_out_order_idx']) ? $arr['sangjo_out_order_idx'] : 0;

    debug_to_file("flower_out_order_idx: $flower_out_order_idx, sangjo_out_order_idx: $sangjo_out_order_idx", 'VARIABLES');
    ...
    if(count($arr['flower_product_list']) > 0 ){
        ...
        for ($i=0;$i<count($flower_product_list);$i++ )
        {
            ...
            $sql_inin = "insert into ".$db_flower.".out_order_client_product 
                set
                ".addConditionalField('out_order_idx', $out_order_idx)."
                ".addConditionalField('consulting_idx', $arr['consulting_idx'])."
                ".addConditionalField('client_product_idx', $flower_product_list[$i]['client_product_idx'])."
                ".addConditionalField('product_idx', $flower_product_list[$i]['product_idx'])."
                ".addConditionalField('product_name', $flower_product_list[$i]['product_name'])."
                ".addConditionalField('option_name', $flower_product_list[$i]['option_name'])."
                ".addConditionalField('client_price', $flower_product_list[$i]['client_price'])."
                ".addConditionalField('client_price_tax', $flower_product_list[$i]['client_price_tax'])."
                ".addConditionalField('client_price_sum', $flower_product_list[$i]['client_price_sum'])."
                ".addConditionalField('order_count', $flower_product_list[$i]['cnt'])."
                ".addConditionalField('total_client_price', $tcp)."
                ".addConditionalField('total_client_price_tax', $tcpt)."
                ".addConditionalField('total_client_price_sum', $tcps)."
                ".addConditionalField('admin_idx', $admin_info['admin_idx'])."
                admin_name='".$admin_info['admin_name']."'"; // 마지막 필드는 콤마 없이




            $in = mysqli_query($dbcon, $sql_inin) or die("주문 상품 저장 중 오류가 발생했습니다. 오류내용: " . mysqli_error($dbcon) . " SQL: " . $sql_inin);
        ...
        if(isset($_FILES['files']['name'])){
            ...
            // 디버깅을 위한 로그 기록
            debug_to_file($_FILES, 'FILES_DATA');
            debug_to_file(count($_FILES['files']['name']), '업로드 파일 수');
            ...
            debug_to_file($absolute_folder_idx, 'UPLOAD_DIRECTORY');
        
            // 디렉토리 생성 시 오류 처리 추가
            if (!file_exists($absolute_folder_idx)) {
                if (!mkdir($absolute_folder_idx, 0777, true)) {
                    debug_to_file("디렉토리 생성 실패: " . $absolute_folder_idx, 'MKDIR_ERROR');
                    // 디렉토리 생성 실패는 치명적 오류가 아니므로 계속 진행
                }
            }
            ...
            for($index = 0;$index < $countfiles;$index++){
                ...
                if(isset($_FILES['files']['name'][$index]) && $_FILES['files']['name'][$index] != ''){
                    ...
                    // 파일명에서 특수문자 제거 및 안전한 파일명으로 변환
                    $filename = preg_replace("/[^a-zA-Z0-9가-힣._-]/", "", $filename);
                    
                    // 파일명이 비어있으면 기본값 설정
                    if (empty($filename)) {
                        $filename = "file_" . time() . "_" . $index;
                    }
                    ...
                    $unvalid_ext = array("exe", "php", "js", "html", "htm");
                    ...
                    if(!in_array($ext, $unvalid_ext)){
                        ...
                        if(file_exists($absolute_path)){ 
                            ...
                            $absolute_path = $upload_location . $new_filename;
                        }
                        ...
                        if(move_uploaded_file($_FILES['files']['tmp_name'][$index], $absolute_path)){
                            ...
                            error_log("파일 업로드 성공: " . $absolute_path);
                        } else {
                            error_log("파일 업로드 실패: " . $absolute_path . " - 오류: " . error_get_last()['message']);
                        }
                    } else {
                        error_log("허용되지 않는 파일 확장자: " . $ext);
                    }
                }
            }
    ...
    if(count($arr['sangjo_product_list']) > 0 ){
        ...
        for ($i=0;$i<count($sangjo_product_list);$i++ )
        {
            ...
            // 카테고리 정보 가져오기
            $sel_oocp = mysqli_query($dbcon, "select b.category1_idx, b.t_category1_name from ".$db_sangjo_new.".client_product a left join ".$db_sangjo_new.".product b on a.product_idx=b.product_idx where a.client_product_idx='".$sangjo_product_list[$i]['client_product_idx']."' ") or die(mysqli_error($dbcon));
            ...
            $in = mysqli_query($dbcon, "insert into ".$db_sangjo_new.".out_order_client_product 
                ...
                consulting_idx='".$arr['consulting_idx']."',
                category1_idx='".$category1_idx."',
                t_category1_name='".$t_category1_name."',
                client_product_idx='".$sangjo_product_list[$i]['client_product_idx']."',
                order_date='".$arr['order_date']."',
                ...
                total_client_price='".($sangjo_product_list[$i]['client_price'] * $sangjo_product_list[$i]['cnt'])."',
                total_client_price_tax='".($sangjo_product_list[$i]['client_price_tax'] * $sangjo_product_list[$i]['cnt'])."',
                total_client_price_sum='".($sangjo_product_list[$i]['client_price_sum'] * $sangjo_product_list[$i]['cnt'])."',
    ...
    try {
        ...
        // 디버깅을 위한 로그 기록
        debug_to_file("트랜잭션 커밋 성공", 'COMMIT_SUCCESS');
        
        $result = array();
        $result['status'] = 1;
        $result['data'] = isset($data) ? $data : array();
        $result['msg'] = "저장되었습니다";
        debug_to_file($result, 'FINAL_RESULT');
        echo json_encode($result);
    } catch (Exception $e) {
        // 오류 발생 시 롤백
        mysqli_rollback($dbcon);
        ...
        // 디버깅을 위한 로그 기록
        debug_to_file("트랜잭션 처리 중 오류 발생: " . $e->getMessage(), 'TRANSACTION_ERROR');
        
        $result = array();
        $result['status'] = 0;
        $result['msg'] = "저장 중 오류가 발생했습니다: " . $e->getMessage();
        debug_to_file($result, 'ERROR_RESULT');
        echo json_encode($result);
    }

} catch (Exception $e) {
    debug_to_file("전체 스크립트 오류: " . $e->getMessage() . "\n" . $e->getTraceAsString(), 'GLOBAL_ERROR');
    
    $result = array();
    $result['status'] = 0;
    $result['msg'] = "처리 중 오류가 발생했습니다: " . $e->getMessage();
    echo json_encode($result);
    exit;
}
