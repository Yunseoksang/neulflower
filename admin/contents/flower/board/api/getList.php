...
//총수주금액
$sel = mysqli_query($dbcon, "select sum(total_client_price_sum) as total_client_price_sum from flower.out_order where ".$sql_period." and  (out_order_status!='배송완료' and out_order_status!='주문취소' ) ") or die(mysqli_error($dbcon));
$sel_num = mysqli_num_rows($sel);

if ($sel_num > 0) {
    $data = mysqli_fetch_assoc($sel);
    $total_client_price_sum = $data['total_client_price_sum'];
}
...
//총수주금액
$total_sql = "select sum(X.total_client_price_sum) as total_client_price_sum from (".$sql_join.") X";

$query_result_x=mysqli_query($dbcon, $total_sql) or die(mysqli_error($dbcon));
$data_x = mysqli_fetch_assoc($query_result_x);
$total_client_price_sum = $data_x['total_client_price_sum'];
...
$result['total_client_price_sum'] = $total_client_price_sum;
