...
if($_POST['mode'] == "del"){
    ...
    if($_POST['out_order_part'] == "상조"){
        ...
        $del_num = mysqli_affected_rows($dbcon);
        if($del_num >= 0){ //쿼리 성공 // 0이면 쿼리성공이지만 변경데이터 없음.
            $sel1 = mysqli_query($dbcon, "select product_name,count(*) as total_order_kinds, sum(order_count) as total_order_count,
            sum(client_price*order_count) as total_client_price,
            sum(client_price_tax*order_count) as total_client_price_tax,
            sum(total_client_price_sum) as total_client_price_sum from ".$db_flower.".out_order_client_product where out_order_idx='".$out_order_idx."' ") or die(mysqli_error($dbcon));
            $sel1_num = mysqli_num_rows($sel1);
            ...
            if ($sel1_num > 0) {
                ...
                total_client_price='".$data1['total_client_price']."',
                total_client_price_tax='".$data1['total_client_price_tax']."',
                ...
                if($up_num >= 0){ //업데이트 성공 // 0이면 쿼리성공이지만 변경데이터 없음.
                    ...
                    $result['data']['total_client_price'] = $data1['total_client_price'];
                    $result['data']['total_client_price_tax'] = $data1['total_client_price_tax'];
                    $result['data']['total_client_price_sum'] = $data1['total_client_price_sum'];
                    $result['data']['order_product_title'] = $data1['product_name'].$product_name_add;
    ...
    if($_POST['out_order_part'] == "상조"){
        ...
        $client_price_sum = $_POST['client_price'] + $_POST['client_price_tax'];
        $total_client_price = $_POST['client_price'] * $_POST['order_count'];
        $total_client_price_tax = $_POST['client_price_tax'] * $_POST['order_count'];
        $total_client_price_sum = $total_client_price + $total_client_price_tax;
        ...
        client_price_sum=".$client_price_sum.",
