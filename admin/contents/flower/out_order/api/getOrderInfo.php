...
$sel = mysqli_query($dbcon, "select a.*,b.manager,b.hp,b.address from ".$db_flower.".out_order a

left join ".$db_flower.".storage b
...
if($_POST['out_order_part'] == "화훼"){
   $sel1 = mysqli_query($dbcon, "select * from ".$db_flower.".out_order_client_product where out_order_idx='".$_POST['out_order_idx']."' ") or die(mysqli_error($dbcon));
...
$sel2 = mysqli_query($dbcon, "select out_order_idx,filename from ".$db_flower.".attachment where out_order_idx='".$data['out_order_idx']."' and attachType='attachment' order by attachment_idx") or die(mysqli_error($dbcon));
...
$sel3 = mysqli_query($dbcon, "select out_order_idx,filename from ".$db_flower.".attachment where out_order_idx='".$data['out_order_idx']."' and attachType='photo' order by attachment_idx") or die(mysqli_error($dbcon));
