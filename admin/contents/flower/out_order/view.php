...
$folder_name = "flower/storage_out";  //폴더명
$title ="출고지시서 보기";

$table_name ="flower.in_out";
...
$main_filter_query = "part='출고' and (io_status != '출고완료' or (receive_date is not null and DATEDIFF(now(),receive_date) < 1))";
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["받는분이름","to_name","like"]);   //로딩시 첫 옵션 기본선택됨. dataTables.ajax.button.js 에서 keyword_query = setKeywordQuery(0); 에 의해 첫번째 옵션이 기본옵션으로 선택됨.
...
$other_table_column = "a:io_idx:out_order:out_order_idx+storage_idx+t_storage_name+sender_name+to_address+to_name+to_hp+to_phone+receiver_name"; //메인테이블이 아닌 다른 테이블에서 join을 통해 가져와야 하는 칼럼 => sorting 할때 뒤쪽에 붙여야 함.
