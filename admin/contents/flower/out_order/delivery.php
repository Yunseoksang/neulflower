...
if($_REQUEST['mode'] == "req"){
    $main_filter_query = "part='출고' and io_status = '미확인' ";
}
...
array_push($th_info,["io_status","진행상태","selectDropDown",[  ["미확인","미확인","btn-info"],["주문취소","주문취소","btn-info"],["본부접수","본부접수","btn-info"],["주문접수","주문접수","btn-warning"],["배송중","배송중","btn-success"],["배송완료","배송완료","btn-dark"] ] ]);

//out_order_idx+r_date+company_name+order_product_title+address1+address2+total_client_price_sum+io_status
...
$other_table_column = "a:out_order_idx:out_order:out_order_idx+out_order_part+r_date+t_company_name as company_name+order_product_title+address1+address2+msgTitle+sender_name+r_hour+total_client_price_sum+date_format(regist_datetime,'%Y-%m-%d %H;%i') as reg_datetime+date_format(r_date,'%m-%d') as r_md";
...
array_push($css_array,["io_status",["width","90px"]]);
...
function datatableCreatedCell(column,$td,rowData){
  switch (column) {
    case "out_order_idx":
        ...
        $td.closest("tr").attr("out_order_idx",rowData.out_order_idx);
