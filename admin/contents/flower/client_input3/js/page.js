...
function select_li($this){
            ...
            if($this.find(".icheckbox_flat-green").hasClass("checked")){
                ...
                $("ul.ul_product_list_right").find("li[client_product_idx='"+client_product_idx+"'][product_part='"+product_part+"'][product_idx='"+product_idx+"']").remove();
            }else{
                $this.find(".icheckbox_flat-green").addClass("checked");
    
                if($("ul.ul_product_list_right").find("li[client_product_idx='"+client_product_idx+"'][product_part='"+product_part+"'][product_idx='"+product_idx+"']").length == 0){
...
$(document).ready(function(){
    ...
    // $(document).on("click",".icheckbox_flat-green",function() {
    //     select_li($(this));
    // });
    ...
    $(document).on("change","select[name='option_price']",function() {
        ...
        if(option_price == "직접입력"){ //직접입력
            $(this).closest("div").html("<input name='option_price' class='form-control' >");
        }
    ...
    $(document).on("keyup","input[name='option_price']",function() {
    ...
    $(document).on("click","button.btn_save_client_order,button.btn_save_out_complete",function() {
        ...
        $("ul.ul_product_list_right").find("li[product_part='flower']").each(function(index) {
            ...
            if($(this).find("select[name='options']").length > 0){
                var option_name = $(this).find("select[name='options'] > option:selected").val();
            ...
            var client_price_sum = parseInt($(this).attr("client_price_sum")) + option_price;
            var client_price     = parseInt($(this).attr("client_price")) + option_price;
            ...
            if(client_price_tax > 0){
                var client_price_tax_calcu = (client_price_tax+option_price*0.1)*cnt;

            }else{
                var client_price_tax_calcu = 0;
...
function saveClientOrderUpload(dataArray){
    ...
    var url = "./contents/flower/client_input/api/save_client_order_upload.php";
    ...
    jQuery.ajax({
        ...
        , success: function(result){
            ...
            if(result_status == 1)
            {
                ...
                window.location.href='?page=flower/out_order/list&mode=order';
...
function reset_total(){
    ...
    $("ul.ul_product_list_right").find("li:not(.li_right_header)").each(function(index) {
        ...
        total += cnt;
        total_client_price += client_price*cnt;
...
function load_client_product(consulting_idx){
    ...
    var url = "./contents/flower/client_input/api/get_client_product_info.php";
            ...
            success: function(result){
                ...
                if(result_status == 1)
                {
                    ...
                    for(var i=0;i<obj.product_list.length;i++){
                        ...
                        $product_li.find(".client_price").text(number_format(pl.client_price));

                        $product_li.find(".client_price_tax").text(number_format(pl.client_price_tax));

                        $product_li.attr("client_price_sum",pl.client_price_sum).attr("client_price",pl.client_price).attr("client_price_tax",pl.client_price_tax);
...
function save_add_sender_name(mode,sender_name,client_flower_sender_idx){
    ...
    var url = "./contents/flower/client_input/api/save_add_sender_name.php";
