...
//********************************************************************************************************************************************************************************** */
$time_check .= "select column_name as columns from common.all_columns where db_name='{$db_name}' and table_name='{$table_name}' :".microtime(true).":".(microtime(true) - $start_check_time);
$time_check .= "\n";
...
$sql_check = "select * from common.all_columns where db_name='{$db_name}' and table_name='{$table_name}' ";
...
if ($sel_num > 0) {
   ...
   while($data = mysqli_fetch_assoc($sel)) {
      ...
      $column_list[$data['column_name']] = $table_name;
...
$sql_query0 = "select  ".$cnt_column." from ".$table_name." where 1=1 ".$rData['filter_query'];

echo $sql_query0;
exit;

$sel = mysqli_query($dbcon, $sql_query0) or die(mysqli_error($dbcon));
