...
// 필터 쿼리를 외부에서 받는 경우, 입력을 검증하고 살균 처리
$filter_query = $dbcon->real_escape_string($rData['filter_query']);

// SQL 쿼리를 안전하게 구성
$query = "SELECT {$cnt_column} FROM {$table_name} WHERE 1=1 {$filter_query}";

// 쿼리 실행
$result = $dbcon->query($query);

if (!$result) {
    // 오류 발생시, mysqli_error()를 로그 파일에 기록하고, 사용자에게는 일반 오류 메시지 표시
    error_log('Database query error: ' . $dbcon->error);  // 로그에 기록
    echo "An error occurred. Please try again later.";    // 사용자에게 안전한 메시지 제공
    //echo $dbcon->error;
    //exit;
}
