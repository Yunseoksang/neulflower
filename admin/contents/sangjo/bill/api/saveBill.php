...
if($bill_idx){//쿼리성공
    ...
    for ($i=0;$i<count($oocp_idx_list);$i++ )
    {
        ...
        $sql_up = "update sangjo_new.out_order_client_product set bill_idx=".$bill_idx.",bill_yyyymm='".$yyyymm."'
    ...
    if($bill_success_num > 0){
        ...
        $sel_check = mysqli_query($dbcon, "select * from sangjo_new.out_order_client_product where bill_idx='".$bill_idx."'  ") or die(mysqli_error($dbcon));
