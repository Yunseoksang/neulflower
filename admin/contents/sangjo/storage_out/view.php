...
$main_filter_query = "part='출고' and io_status != '출고취소' and (io_status != '출고완료' or (receive_date is not null and DATEDIFF(now(),receive_date) < 1))";
...
<!-- 라이트 박스 -->
  ...
  <p class="hide"><a id='inline_qr' class='inline_qr' href="#inline_content">Inline HTML</a></p>
        ...
        <a id="btn-scan-qr" style="width:100%; text-align:center; cursor:pointer;">
          <img class="qr_logo" src="./contents/sangjo/storage_out/img/qrcode.png" title="클릭! QR코드 읽기">
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "io_status":
        ...
        rValue += '<br><button type="button" class="btn btn-info btn_qr_out "  next_io_status="출고취소">QR출고하기</button>';
