...
for ($i=0;$i<count($product_list);$i++ )
{
    ...
    $sel = mysqli_query($dbcon, "select * from ".$db_sangjo_new.".storage_safe where storage_idx='".$arr['storage_idx']."' and product_idx='".$product_list[$i]['product_idx']."' ") or die(mysqli_error($dbcon));
    ...
    $sel_oocp = mysqli_query($dbcon, "select * from ".$db_sangjo_new.".out_order where oocp_idx='".$product_list[$i]['oocp_idx']."'  ") or die(mysqli_error($dbcon));
    ...
    $in = mysqli_query($dbcon, "insert into ".$db_sangjo_new.".out_order
    set 
    ".$oocp_idx_sql."
    consulting_idx='".($arr['consulting_idx'] ?: 0)."',
    ...
    if($out_order_idx){//쿼리성공
        ...
        $up = mysqli_query($dbcon, "update ".$db_sangjo_new.".out_order_client_product 
        set out_order_idx='".$out_order_idx."'
        where oocp_idx='".$product_list[$i]['oocp_idx']."'") or die(mysqli_error($dbcon));
        ...
        if(isset($arr['agency_order_price']) && $arr['agency_order_price'] != ""){
            $additional_sql .= "agency_order_price='".($arr['agency_order_price'] ?: 0)."',";
        }


        $sel = mysqli_query($dbcon, "select * from ".$db_sangjo_new.".client_product where client_product_idx='".$product_list[$i]['client_product_idx']."'  ") or die(mysqli_error($dbcon));
        $sel_num = mysqli_num_rows($sel);
        ...
        $in = mysqli_query($dbcon, "insert into ".$db_sangjo_new.".in_out 
        set
        consulting_idx='".($arr['consulting_idx'] ?: 0)."',
        oocp_idx='".($product_list[$i]['oocp_idx'] ?: 0)."',
        storage_idx='".($arr['storage_idx'] ?: 0)."',
        out_order_idx='".($out_order_idx ?: 0)."',
        product_idx='".$product_idx."',
        client_product_idx='".($product_list[$i]['client_product_idx'] ?: 0)."',
        ...
        if($io_idx){//쿼리성공
            ...
            //안전재고 테이블 합계 저장 업데이트
            $in = mysqli_query($dbcon, "insert into ".$db_sangjo_new.".storage_safe (storage_idx,product_idx,current_count) values ('".$arr['storage_idx']."','".$product_idx."','".$next_count."')
    ...
    if($arr['move_check'] == 1){ //이동지시서 작성
        ...
        $sel = mysqli_query($dbcon, "select * from ".$db_sangjo_new.".product where product_idx='".$product_list[$i]['product_idx']."' ") or die(mysqli_error($dbcon));
        $sel_num = mysqli_num_rows($sel);
        ...
        $sel = mysqli_query($dbcon, "select * from ".$db_sangjo_new.".storage_safe where storage_idx='".$base_storage_idx."' and product_idx='".$product_list[$i]['product_idx']."'  ") or die(mysqli_error($dbcon));
        ...
        client_product_idx='".$product_list[$i]['client_product_idx']."',
        part='이동출고',
        io_status='미출고',
        out_count='".$product_list[$i]['cnt']."',
