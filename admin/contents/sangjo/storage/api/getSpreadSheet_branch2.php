...
$filename = date("지점별재고현황2_Ymd_His",time()).".xlsx";
...
// 대성 지사 합계 (E열)
$sheet->setCellValue('E1', '지사합계(대성)');
$sheet->setCellValue('E2', $non_douzone_sum_total);

// 대성 지사별 데이터 (F열부터)
for ($i=0; $i<count($non_douzone_storage_arr); $i++) {
    $col_num = $i+5; // F열부터 시작
    $last_cell_name = $column_name_arr[$col_num]."1";
    $sheet->setCellValue($last_cell_name, $non_douzone_storage_arr[$i]['storage_name']);

    $storage_column = "storage_idx_".$non_douzone_storage_arr[$i]['storage_idx'];
...
for ($i=0; $i<count($product_arr); $i++) {
    ...
    if ($current_group != $product_arr[$i]['display_group'] && 
        ...
        // 그룹 헤더 스타일 설정
        $last_column = $column_name_arr[count($non_douzone_storage_arr)+4]; // 본사, 지사합계 컬럼 고려
    ...
    $sheet->setCellValue("E".$row_num, $non_douzone_product_sum);
    
    // 대성 지사별 데이터
    for ($k=0; $k<count($non_douzone_storage_arr); $k++) {
        $storage_column = "storage_idx_".$non_douzone_storage_arr[$k]['storage_idx'];
        $cell_value = "";
...
// 스프레드시트 스타일 적용
$spreadsheet = setSpreadSheetStyle($spreadsheet, $sheet, $last_cell_name, $row_num);

// 지점별 컬럼 너비 설정
setExtraColumnWidths($sheet, 3, count($non_douzone_storage_arr));
