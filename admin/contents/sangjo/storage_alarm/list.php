...
$folder_name = "sangjo/storage_alarm";  //폴더명
$title ="안전재고 알림";

$table_name ="sangjo_new.storage_safe";
...
$main_filter_query = " current_count < safe_count ";
...
array_push($th_info,["st_safe_idx","고유번호"]);
array_push($th_info,["t_storage_name","창고명"]);
array_push($th_info,["t_product_name","품목"]);
array_push($th_info,["current_count","현재수량"]);
array_push($th_info,["safe_count","안전수량"]);
array_push($th_info,["exec_need_count","부족수량"]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "exec_need_count":
      
          if(isNaN(full.safe_count)){
            var safe_count = 0;
          }else{
            var safe_count = parseInt(full.safe_count);
          }

          if(isNaN(full.current_count)){
            var current_count = 0;
          }else{
            var current_count = parseInt(full.current_count);
          }

          var needed_num = safe_count - current_count;
          if(needed_num < 0){
            needed_num = 0;
          }
