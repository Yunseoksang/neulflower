...
if($admin_info['pm_super'] == "종합관리자"){
  //
}else if($admin_info['pm_hrm'] == "총무관리자" || $admin_info['pm_hrm'] == "인사총무관리자" ){
  // 
}else{
...
$function_add_html = '<a href="?page=cmu/input"><button class="btn btn-primary btn_add_custom btn_add_supplier">총무관리 추가</button></a>';


if($_REQUEST['status'] == "1"){
  $main_filter_query = "cmu_status='관리중' ";
}else if($_REQUEST['status'] == "2"){
  $main_filter_query = "cmu_status='종료' ";
}else{
  $main_filter_query = "cmu_status='관리중'  ";
}
...
if($function_date_search == "on"){
  ...
  $function_date_column['update_datetime'] = "수정일";  //날짜 검색 옵션칼럼 + 보여지는문구
  //-------------------------------------------------------------------------
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["지사","t_jisa","like"]); 
  array_push($keyword_column_array,["업체명","c_company","like"]); 
  array_push($keyword_column_array,["관리명","title","like"]); 
  ...
  $keyword_column_array_selected = "title";      //기본 선택옵션 value
...
array_push($th_info,["cmu_idx","고유번호"]);
array_push($th_info,["title","관리명"]);
array_push($th_info,["t_jisa","담당지사"]);
array_push($th_info,["c_company","담당업체"]);
array_push($th_info,["expense","비용"]);
array_push($th_info,["payment_date","결제일"]);

array_push($th_info,["cmu_status","상태","selectDropDown",[ ["관리중","관리중","btn-success"],["종료","종료","btn-dark"] ] ]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "title":
          ...
          rValue = "<a href='?page=cmu/detail&cmu_idx="+full.cmu_idx+"'>"+full.title+"</a>";
