...
//     $location = $_SERVER["DOCUMENT_ROOT"].'/upload/cmu/'.$name;
...
//$path = $_SERVER["DOCUMENT_ROOT"]."/upload/cmu/";
...
if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    {       
        ...
        $folder_idx = "/upload/cmu/".$_POST['cmu_idx'];
		...
		if(file_exists($absolute_path)){ 
            ...
            $local_name = "/upload/cmu/".$_POST['cmu_idx']."/".$new_name;
            $absolute_path = $_SERVER["DOCUMENT_ROOT"].$local_name;
        }
        ...
        if(move_uploaded_file($tmp, $absolute_path))
        {
            ...
            if($in_id){//쿼리성공
                ...
                if ($sel_num > 0) {
                    ...
                    $up = mysqli_query($dbcon, "update cmu.cmu set update_datetime=now() where cmu_idx='".$_POST['cmu_idx']."' ") or die(mysqli_error($dbcon));
