...
$folder_name = "sj_local/storage_out";  //폴더명
$title ="[".$admin_info['t_storage_sangjo']."] 출고지시서 보기";
...
$main_filter_query = "storage_idx='".$admin_info['storage_sangjo']."' and part='출고' and io_status='미출고'";
...
if($function_keyword_search == "on"){
  ...
  $keyword_column_array_selected = "to_name";      //기본 선택옵션 value
...
if($this_agent == "pc"){
  ...
  $unsortable_columns = [];  //soring 기능 불가칼럼 지정.
  ...
  $unsortable_columns = ["t_product_name","out_count","to_place_name","regist_datetime","io_status"];  //soring 기능 불가칼럼 지정.
...
array_push($css_array,["to_address",["min-width","200px"]]);
...
array_push($edit_array,["out_date","출고일","","text"]);
...
$add_array = [];
array_push($add_array,["modal_info",["new","인수자 등록",""]]); //0: 모달창네임, 1: 모달창타이틀, 2: submit_url 3:적용할 table, 4: 적용할 row 고유키명
...
array_push($add_array,["receiver_name","인수자","필수","text"]);
...
?>
  ...
  <div id="dialog" title="출고완료 처리">
  <p class="validateTips"></p>
 
    <fieldset>
      <div class="col-md-3 col-sm-3 col-xs-3">
      <label for="name">인수자</label>
    </div>
    <div  class="col-md-9 col-sm-9 col-xs-9">
       <input type="text" name="receiver_name" id="receiver_name" value="" class="text ui-widget-content ui-corner-all" >

    </div>
      <input type="hidden" name="io_idx" value="">
      <p class="validateTips" style="clear:both;"></p>
      

 
      <div class="div_btn"><button class="btn btn-dark btn_modal_close">닫기</button><button class="btn btn-success btn_io_status_save">출고완료</button></div>
    </fieldset>
</div>
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "exec_detail":
        ...
        rValue += "<div class='detail_div hide'>"+full.io_idx + " | " +full.to_place_name+" | "+full.to_name+" | "+full.to_hp+" | "+full.to_address+" | "+full.memo;
        if (full.receiver_name != undefined || full.receiver_name != ""){
          rValue += " | <span class='receiver_name'>"+full.receiver_name+"</span>";
        }
        rValue += "</div>";
        break;
    ...
    case "io_status":
        <?
        if($this_agent == "pc"){?>
        rValue = '<button type="button" class="btn btn-danger btn_io_status"  next_io_status="출고완료">'+data+'</button>';

        <?}else{//mobile ?>
          rValue = '<button type="button" class="btn btn-xs btn-danger btn_io_status"  next_io_status="출고완료">'+data+'</button>';
