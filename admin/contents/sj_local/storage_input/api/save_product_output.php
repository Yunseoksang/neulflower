...
for ($i=0;$i<count($product_list);$i++ )
{
    ...
    $sel = mysqli_query($dbcon, "select * from storage_safe where storage_idx='".$admin_info['storage_sangjo']."' and product_idx='".$product_list[$i]['product_idx']."' order by io_idx desc limit 1") or die(mysqli_error($dbcon));
    ...
    set
    storage_idx='".$admin_info['storage_sangjo']."',
    ...
    if($in_id){//쿼리성공
        ...
        if($in_id){//쿼리성공
            ...
            //안전재고 테이블 합계 저장 업데이트
            $in = mysqli_query($dbcon, "insert into storage_safe (storage_idx,product_idx,current_count) values ('".$admin_info['storage_sangjo']."','".$product_list[$i]['product_idx']."','".$next_count."')
    ...
    if($arr['move_check'] == 1){ //이동지시서 작성
        ...
        io_status='미출고',
        out_count='".$product_list[$i]['cnt']."',
        to_storage_idx='".$admin_info['storage_sangjo']."',
