...
$folder_name = "sfullfillment/storage_adjust";  //폴더명
$title ="상품별 히스토리";
...
$main_filter_query = " io_status in ('이동출고완료', '이동입고완료', '조정완료', '입고완료', '출고완료') ";

if($_REQUEST['product_idx'] != "" && $_REQUEST['storage_idx'] != ""){
    $main_filter_query = "product_idx='".$_REQUEST['product_idx']."' and storage_idx='".$_REQUEST['storage_idx']."' ";
}else if($_REQUEST['product_idx'] != "" ){
    $main_filter_query = "product_idx='".$_REQUEST['product_idx']."' ";

}else if($_REQUEST['storage_idx'] != ""){
    $main_filter_query = "storage_idx='".$_REQUEST['storage_idx']."' ";
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["제품명(포함)","t_product_name","like"]);
  array_push($keyword_column_array,["제품명(일치)","t_product_name","="]);
  array_push($keyword_column_array,["창고명","t_storage_name","like"]);
  ...
  $keyword_column_array_selected = "t_product_name";      //기본 선택옵션 value
...
$order_by_column = "status_change_datetime";
$order_by_sort = "desc";
...
array_push($th_info,["t_from_storage_name","출발"]);
array_push($th_info,["t_to_storage_name","도착/발주사"]);

array_push($th_info,["t_product_name","상품명"]);
array_push($th_info,["part","작업내역"]);
array_push($th_info,["io_status","진행단계"]);

array_push($th_info,["exec_prev_count","조정전 수량"]);

array_push($th_info,["in_count","증가수량"]);
array_push($th_info,["out_count","감소수량"]);


array_push($th_info,["current_count","조정후 수량"]);
array_push($th_info,["memo","메모"]);


array_push($th_info,["regist_datetime","등록일"]);
array_push($th_info,["status_change_datetime","상태변경일"]);
...
$other_table_column = "a:consulting_idx:consulting.consulting:company_name"; //메인테이블이 아닌 다른 테이블에서 join을 통해 가져와야 하는 칼럼 => sorting 할때 뒤쪽에 붙여야 함.
...
array_push($css_array,["memo",["min-width","130px"]]);

array_push($css_array,["regist_datetime",["min-width","100px"]]);
array_push($css_array,["update_datetime",["min-width","100px"]]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "t_storage_name":
        <? 
            if($_REQUEST['product_idx'] != ""){?>
                rValue = "<a href='?page=sfullfillment/storage_adjust/history&product_idx=<?=$_REQUEST['product_idx']?>&storage_idx="+full.storage_idx+"'>"+data+"</a>";

            <?}else{?>
                rValue = "<a href='?page=sfullfillment/storage_adjust/history&storage_idx="+full.storage_idx+"'>"+data+"</a>";
    ...
    case "t_product_name":
        <? 
            if($_REQUEST['storage_idx'] != ""){?>
                rValue = "<a href='?page=sfullfillment/storage_adjust/history&product_idx="+full.product_idx+"&storage_idx=<?=$_REQUEST['storage_idx']?>'>"+data+"</a>";

            <?}else{?>
                rValue = "<a href='?page=sfullfillment/storage_adjust/history&product_idx="+full.product_idx+"'>"+data+"</a>";

            <?}
        ?>
          break;
    case "t_to_storage_name":
          if(full.part == "출고"){
    ...
    case "in_count":
        rValue = data;
          ...
          break;
    case "exec_prev_count":

        if(full.io_status == "미출고" || full.io_status == "미입고" ||  full.io_status == "이동출고취소" ||  full.io_status == "출고취소"){
            rValue = parseInt(full.current_count);
        }else{
            if(parseInt(full.in_count) > 0){
              rValue = parseInt(full.current_count) - parseInt(full.in_count);

            }else if(parseInt(full.out_count) > 0){
              rValue = parseInt(full.current_count) + parseInt(full.out_count);
    ...
    case "current_count":
        ...
        if(full.io_status == "미출고"){
          rValue = data+" (-"+full.out_count+")";

        }else if(full.io_status == "미입고"){
          rValue = data+" (+"+full.in_count+")";
        }else{
          rValue = data;
        }
    ...
    case "regist_datetime":
      ...
      rValue = data+"<br>("+full.t_write_admin_name+")";
      break;

    case "status_change_datetime":
          var status_change_manager = "";

          if(full.io_status == "이동출고완료"){
            status_change_manager = full.t_moveout_manager_name;
          }else if(full.io_status == "이동입고완료"){
            status_change_manager = full.t_movein_manager_name;
          }else if(full.io_status == "조정완료"){
            status_change_manager = full.t_adjust_manager_name;
          }else if(full.io_status == "입고완료"){
            status_change_manager = full.t_in_manager_name;
          }else if(full.io_status == "출고완료"){
            status_change_manager = full.t_out_manager_name;
          }else if(full.io_status == "입고완료"){
            status_change_manager = full.t_in_manager_name;
          }else if(full.io_status == "출고취소" || full.io_status == "이동출고취소" ){
            status_change_manager = full.t_cancel_manager_name;
          }else{
            status_change_manager = full.t_write_admin_name;
          }

          rValue = data+"<br>("+status_change_manager+")";
