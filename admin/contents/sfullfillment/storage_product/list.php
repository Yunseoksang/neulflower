...
$folder_name = "sfullfillment/storage_product";  //폴더명
$title ="제품관리";
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["중분류","t_category2_name","like"]);  
...
array_push($th_info,["category2_idx","중분류"]);


array_push($th_info,["product_name","제품명"]);
array_push($th_info,["display_order","노출순서"]);
array_push($th_info,["display_group","관리그룹"]);



array_push($th_info,["base_storage_idx","기본출고지","select2",["t_base_storage_name"] ]);
array_push($th_info,["product_price","납품단가"]);
array_push($th_info,["cost","제조원가"]);


array_push($th_info,["sum_current_count","현재재고수량"]);
array_push($th_info,["specifications","규격"]);
array_push($th_info,["unit","단위"]);
...
//$other_table_column = "a:"; //메인테이블이 아닌 다른 테이블에서 join을 통해 가져와야 하는 칼럼 => sorting 할때 뒤쪽에 붙여야 함.
...
array_push($edit_array,["product_price","number"]);
array_push($edit_array,["display_order","노출순서","","number"]);
array_push($edit_array,["display_group","담당자","","text"]);
array_push($edit_array,["unit","단위","","text"]);

array_push($edit_array,["product_price","납품단가","","text"]);
array_push($edit_array,["cost","제조원가","","text"]);
array_push($edit_array,["specifications","규격","","text"]);
...
array_push($edit_array,["category1_idx","select2",["options",$category1_idx_array],["direct_save","1"],["same","N"]]);

$category2_idx_array = [["","--선택--"]];

array_push($edit_array,["category2_idx","select",["options",$category2_idx_array],["direct_save","1"],["same","N"]]);
...
$category2_idx_array = [['0','-선택-']];
array_push($add_array,["category2_idx","중분류","","select",$category2_idx_array  ]); //DB column 기준이라서 brand_name 이 아니고,brand_idx로,
...
array_push($add_array,["product_price","납품단가","필수","number"]);
array_push($add_array,["base_storage_idx","기본출고지","필수","select",$storage_idx_array]); //add form 에서는 select2 버그로 작동 안하고 있음.

array_push($add_array,["display_order","노출순서","필수","number"]);
array_push($add_array,["display_group","관리그룹","필수","text"]);
array_push($add_array,["unit","단위","","text"]);

array_push($add_array,["cost","제조원가","","number"]);
array_push($add_array,["specifications","규격","","text"]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    ...
    case "category1_idx":
      ...
      rValue = full.t_category1_name;
      break;

    case "category2_idx":

      rValue = full.t_category2_name;
      break;

    case "sum_current_count":
         if (data == ""){
            rValue = 0;
         } else{
          rValue = data;
