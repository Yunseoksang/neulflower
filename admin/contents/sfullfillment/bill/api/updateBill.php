...
if($_POST['mode'] == "bill_month"){ //미발행된 내역의 발행월 조정, 발행제외처리, 발행된 경우에는 수정요청모드 일때인지 체크
    ...
    $sel = mysqli_query($dbcon, "select * from fullfillment.out_order_client_product where oocp_idx='".$_POST['oocp_idx']."' ") or die(mysqli_error($dbcon));
    ...
    if (preg_match('/^(\d{4})(0[1-9]|1[0-2])$/', $_POST['bill_month'])) {
        ...
        $sql = "update fullfillment.out_order_client_product set bill_yyyymm='".$bill_month."'  where oocp_idx='".$_POST['oocp_idx']."' ";

    }else if($_POST['bill_month'] == "발행제외"){
        //    
        $sql = "update fullfillment.out_order_client_product set bill_yyyymm='-1'  where oocp_idx='".$_POST['oocp_idx']."' ";
    ...
    if($up_num >= 0){ //업데이트 성공 // 0이면 쿼리성공이지만 변경데이터 없음.
        ...
        $up = mysqli_query($dbcon, "update fullfillment.out_order_client_product set bill_idx=null  where  bill_idx='".$_POST['bill_idx']."'
        and consulting_idx='".$_POST['consulting_idx']."' ") or die(mysqli_error($dbcon));
        $up_num = mysqli_affected_rows($dbcon);
        if($up_num >= 0){ //업데이트 성공 // 0이면 쿼리성공이지만 변경데이터 없음.
    ...
    if($_POST['bigo'] == ""){
        $up = mysqli_query($dbcon, "update fullfillment.out_order_client_product set bigo=null
        ...
        $up = mysqli_query($dbcon, "update fullfillment.out_order_client_product set bigo='".$_POST['bigo']."'
        where oocp_idx='".$_POST['oocp_idx']."'
        ") or die(mysqli_error($dbcon));
        $up_num = mysqli_affected_rows($dbcon);
        if($up_num >= 0){ //업데이트 성공 // 0이면 쿼리성공이지만 변경데이터 없음.
