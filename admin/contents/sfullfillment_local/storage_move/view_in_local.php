...
$title ="[".$admin_info['t_storage_fullfillment']."] 이동입고";
...
$main_filter_query = " (storage_idx='".$admin_info['storage_fullfillment']."' and  part='이동입고' and io_status = '미입고'   )";  //and DATEDIFF(now(),receive_date) < 1
...
if($function_keyword_search == "on"){
  ...
  array_push($keyword_column_array,["출발지창고명","t_from_storage_name","like"]);   //로딩시 첫 옵션 기본선택됨. dataTables.ajax.button.js 에서 keyword_query = setKeywordQuery(0); 에 의해 첫번째 옵션이 기본옵션으로 선택됨.
  ...
  $keyword_column_array_selected = "t_from_storage_name";      //기본 선택옵션 value
...
if($this_agent == "pc"){
  ...
  //array_push($th_info,["io_status","처리상태","selectDropDown",[  ["미입고","미입고","btn-primary"],["이동입고완료","이동입고완료","btn-dark","divider"]    ] ]);
...
$opt = ":--처리상태--/미출고/이동출고완료/이동입고완료";  //"0:노출중단/1:노출"
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    case "exec_detail":
        ...
        rValue += "<div class='detail_div hide'>"+full.io_idx+" | " +full.t_from_storage_name+" → "+full.t_storage_name+" | 요청일:" +full.regist_datetime.split(" ")[0]+" | "+full.memo;
        rValue += "</div>";
        break;
    case "io_idx":
          rValue = data;
