...
$title ="[".$admin_info['t_storage_fullfillment']."] 이동출고";
...
$main_filter_query = " (storage_idx='".$admin_info['storage_fullfillment']."' and  part='이동출고' and io_status = '미출고'   )";  //and DATEDIFF(now(),receive_date) < 1
...
if($function_keyword_search == "on"){
  ...
  $keyword_column_array_selected = "t_to_storage_name";      //기본 선택옵션 value
...
if($this_agent == "pc"){
  ...
  //array_push($th_info,["io_status","처리상태","selectDropDown",[  ["미출고","미출고","btn-primary"],["이동출고완료","이동출고완료","btn-success"]   ] ]);
...
function datatableRender(column,data,full){
  ...
  switch (column) {
    case "exec_detail":
        ...
        rValue += "<div class='detail_div hide'>"+full.io_idx+" | " +full.t_from_storage_name+" → "+full.t_storage_name+" | "+full.memo;
        rValue += "</div>";
        break;
    case "io_idx":
          rValue = data;
