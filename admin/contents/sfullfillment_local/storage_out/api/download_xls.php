...
if($_REQUEST['mode'] == "list"){
    ...
    $where_sql = "storage_idx='".$admin_info['storage_fullfillment']."'  and part='출고' and io_status='출고완료' ";

}else if($_REQUEST['mode'] == "view_delivery"){
    $filename = "출고지시서목록_".$todate;

    $where_sql = "storage_idx='".$admin_info['storage_fullfillment']."'  and part='출고' and io_status='미출고' ";
...
from (select * FROM in_out where ".$where_sql." ORDER BY io_idx desc ) a left join out_order on a.out_order_idx=out_order.out_order_idx where 1=1 order by io_idx desc ") or die(mysqli_error($dbcon));
...
//->setCellValue('B'.$num, $row['regist_datetime'])

// ->setCellValue('C'.$num, $row['t_storage_name'])
// ->setCellValue('G'.$num, $row['to_place_name'])
// ->setCellValue('J'.$num, $row['to_phone'])
// ->setCellValue('L'.$num, $row['receiver_name'])
// ->setCellValue('M'.$num, $row['out_date'])
// ->setCellValue('O'.$num, $row['t_update_admin_name'])
// ->setCellValue('P'.$num, $row['io_status'])
;
