   ...
   $(document).on("click","button.btn_view_group",function(e){
    ...
    function updateRow($clickedObj,mode){ //mode == tr 일때만 마지막 button cell 복구 작업 수행.
        ...
        $tar.find("input:not([type='file']),textarea,select").not(".temp").each(function(){
            ...
            if (typeof old_value !== "undefined" && old_value !== "" && new_value !== null && typeof new_value !== "undefined") {
                if (new_value.replace(',', '') === old_value.replace(',', '')) { // 숫자는 컴마 제외한 값 비교, 값의 변화가 없으면.
            ...
            //$this_td.attr("new_value",new_value); //단일 폼용.
                ...
                if ($(this).get(0).tagName === "SELECT" && $(this).attr("same") === "N") { // select 박스인데 value와 text가 다른 경우
                    ...
                    if ($(this).hasClass("select2_single")) {
                        ...
                        new_value_text = $(this).find("option:selected").text(); // select2의 경우

                        console.log('new_value_text3:'+new_value_text);
                    } else {

                        console.log("차례:4");

                        new_value_text = $(this).find("option:selected").text(); // 일반 select 박스의 경우
                        console.log('new_value_text4:'+new_value_text);
...
function makeAddFormMulti(add_array){
    ...
    */
    ...
    for(var i=1;i<add_array.length;i++){
        ...
        if(input_type == "text" || input_type == "number" || input_type == "textarea" || input_type == "hidden"){
...
function convert_form($td,mode=''){
    ...
    //==> 아래 each 문에서 호출함. 모든 td를 순회하면서 작동함
    // $tr.find("td:visible.selectable,td:visible.editable:not(.editable_img),td:visible.editable_custom").each(function(){
    //     //console.log("each테스트:"+$(this).attr("column"));
    //     convert_form( $(this),"edit");
    // });
    ...
    if(mode == "edit"){
        ...
        // Set the initial value of select2
        if (old_value) {
            $td.find(".select2_single").val(old_value).trigger('change');
            $td.find("select").val(old_value).trigger('change');
                    ...
                    if(cnt_selectDropDown.length > 0){ 
                        ...
                        if(old_value == undefined){
                            ...
                            if($(this).get(0).tagName == "SELECT" && $(this).attr("same") == "N"){
                                ...
                                if(old_value == undefined){
                                    old_value = $td.attr(column);
                                    if(old_value == undefined){
                                        $area.html("");

                                    }else{
                                        $area.html($area.find("select option[value='"+old_value+"']").text());
